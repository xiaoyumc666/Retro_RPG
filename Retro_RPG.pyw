                ...
                def handler():
                    ...
                    def safe_restore():
                        try:
                            ...
                            pass
                    self.root.after(2000, safe_restore)
                    cmd()
                return handler
        
            btn.config(command=make_handler(button_config['command']))
...
5. 究极 - 极限挑战，只有最资深的玩家才能生存
    ...
    def start_new_game(self, player_name, magic_affinity, dialog):
        ...
        if difficulty in ['extreme', 'ultimate']:
            # 极难和究极模式：不给任何初始物品
            self.game.add_message(f"⚠️ {difficulty.upper()}模式：你没有任何初始物资，一切都要靠自己！", 'warning')
            if difficulty == 'ultimate':
                self.game.add_message("💀 究极模式：你只有一条命！战败即永久死亡！", 'error')
        else:
            # 普通难度：给初始物品
        ...
        if difficulty not in ['extreme', 'ultimate']:
...
祝您游戏愉快！
    ...
    def show_compendium(self):
        ...
        def toggle_fullscreen(event=None):
            current = dialog.attributes("-fullscreen")
            if current:
                dialog.attributes("-fullscreen", False)
                dialog.geometry("1400x700")
                dialog.update_idletasks()
                x = (dialog.winfo_screenwidth() - 1400) // 2
    ...
    def show_game_interface(self):
        ...
        def show_character_menu():
            ...
            tk.Button(button_frame, text="🗡 装备", command=wrap_command(self.show_equipment_screen), font=self.normal_font, bg=self.colors['button_bg'], fg=self.colors['button_fg'], width=15, height=2).pack(pady=5)
        ...
        buttons_config = [
            ...
            {'text': '🗺 地图', 'command': self.show_map},
            {'text': '⚒️ 合成', 'command': self.game.show_crafting_system},
            {'text': '📚 图鉴', 'command': self.show_compendium},
            {'text': '🗡 装备', 'command': self.show_equipment_screen},
    ...
    def _format_item_stats(self, item_info):
        """格式化物品的实际数值和功能（用于装备界面显示）"""
        if not item_info:
            return ""

        stats = []

        # 所有实际生效的加成属性：(属性键, 显示名, 是否百分比)
        bonus_attrs = [
            ('bonus_attack', '攻击', False),
            ('bonus_defense', '防御', False),
            ('bonus_magic', '魔法', False),
            ('bonus_hp', '生命', False),
            ('bonus_speed', '速度', False),
            ('bonus_crit', '暴击', True),
            ('crit_chance', '暴击', True),
            ('bonus_dodge', '闪避', True),
            ('dodge_chance', '闪避', True),
            ('bonus_block', '格挡', True),
            ('block_chance', '格挡', True),
            ('bonus_thorns', '反伤', True),
            ('thorns', '反伤', True),
            ('lifesteal', '吸血', True),
            ('bonus_gold', '金币', True),
            ('bonus_exp', '经验', True),
            ('bonus_luck', '幸运', False),
            ('bonus_magic_resist', '魔抗', False),
            ('bonus_heal', '治疗', False),
            ('hp_regen', '每回合回血', False),
            ('mana_regen', '每回合回蓝', False),
            ('bonus_all_percent', '全属性', True),
        ]
        for key, label, is_percent in bonus_attrs:
            val = item_info.get(key, 0)
            if val:
                sign = '+' if val > 0 else ''
                suffix = '%' if is_percent else ''
                stats.append(f'{label} {sign}{val}{suffix}')

        # 武器特殊效果
        element_map = {'fire': '火属性', 'ice': '冰属性', 'thunder': '雷属性',
                       'light': '光属性', 'dark': '暗属性', 'wind': '风属性'}
        if 'element' in item_info:
            elem = element_map.get(item_info['element'], item_info['element'] + '属性')
            elem_dmg = item_info.get('element_damage', 0)
            if elem_dmg > 0:
                stats.append(f'{elem}伤害 +{elem_dmg}')
            else:
                stats.append(elem)

        if item_info.get('burn_chance', 0) > 0:
            stats.append(f'燃烧 {item_info["burn_chance"]}%')
        if item_info.get('freeze_chance', 0) > 0:
            stats.append(f'冻结 {item_info["freeze_chance"]}%')
        if item_info.get('effect_chance', 0) > 0:
            et_map = {'paralyze': '麻痹', 'poison': '中毒', 'stun': '眩晕', 'slow': '减速'}
            et_name = et_map.get(item_info.get('effect_type', ''), '特殊效果')
            stats.append(f'{et_name} {item_info["effect_chance"]}%')
        if item_info.get('bonus_against_dark', 0) > 0:
            stats.append(f'对暗系 +{item_info["bonus_against_dark"]}%')
        if item_info.get('chain_lightning'):
            stats.append('连锁雷电')

        # 如果没有任何 bonus 属性，则回退显示基础的 effect + value
        if not stats:
            value = item_info.get('value', 0)
            effect = item_info.get('effect', '')
            if value > 0 and effect:
                effect_map = {
                    'attack': f'攻击 +{value}',
                    'defense': f'防御 +{value}',
                    'hp': f'生命 +{value}',
                    ...
                    'heal': f'恢复生命 +{value}',
                    'mana': f'恢复魔法 +{value}',
                    'hp_regen': f'每回合回血 +{value}',
                    'mana_regen': f'每回合回蓝 +{value}',
                    'lifesteal': f'吸血 +{value}%',
                    'thorns': f'反伤 +{value}%',
                    'block': f'格挡 +{value}%',
                    'crit': f'暴击 +{value}%',
                    'dodge': f'闪避 +{value}%',
                ...
                if effect in effect_map:
                    stats.append(effect_map[effect])

        # 耐久度（消耗品的 durability=1 表示使用次数，不显示）
        item_type = item_info.get('type', '')
        durability = item_info.get('durability', 0)
        if durability > 0 and item_type not in ('consumable',):
            stats.append(f'耐久 {durability}')

        return ' | '.join(stats) if stats else ''

    def _bind_mousewheel(self, canvas):
        """为 canvas 及其所有子控件递归绑定鼠标滚轮滚动。"""
        def _on_wheel(e):
            if e.delta:
    ...
    def show_equipment_screen(self):
        ...
        if weapon_name != '无' and weapon_name in self.game.items:
            ...
            weapon_stats = self._format_item_stats(weapon_info)
            if weapon_stats:
                weapon_stats_label = tk.Label(
                    weapon_frame,
                    text=f"📊 {weapon_stats}",
                    font=self.small_font,
                    fg=self.colors['info'],
                ...
                weapon_stats_label.pack(fill='x', padx=10, pady=(0, 5))
        ...
        if armor_name != '无' and armor_name in self.game.items:
            ...
            armor_stats = self._format_item_stats(armor_info)
            if armor_stats:
                armor_stats_label = tk.Label(
                    armor_frame,
                    text=f"📊 {armor_stats}",
                    font=self.small_font,
                    fg=self.colors['info'],
                ...
                armor_stats_label.pack(fill='x', padx=10, pady=(0, 5))
        ...
        if accessory_name != '无' and accessory_name in self.game.items:
            ...
            accessory_stats = self._format_item_stats(accessory_info)
            if accessory_stats:
                accessory_stats_label = tk.Label(
                    accessory_frame,
                    text=f"📊 {accessory_stats}",
                    font=self.small_font,
                    fg=self.colors['info'],
                    bg='#1e1e1e',
                    anchor='w'
                )
                accessory_stats_label.pack(fill='x', padx=10, pady=(0, 5))
        ...
        if not weapons_list:
            ...
            canvas.configure(yscrollcommand=scrollbar.set)
            canvas.bind('<Configure>', lambda e, c=canvas, w=_win_id: c.itemconfig(w, width=e.width))
            ...
            for item_name, quantity, item_info in weapons_list:
                ...
                # 顶行：名称 + 装备按钮
                top_row = tk.Frame(item_frame, bg='#1e1e1e')
                ...
                desc_label.pack(fill='x', padx=15)

                # 数值与功能
                weapon_stats = self._format_item_stats(item_info)
                if weapon_stats:
                    ...
                    stats_label.pack(fill='x', padx=15, pady=(0, 2))
        ...
        if not armors_list:
            ...
            for item_name, quantity, item_info in armors_list:
                ...
                armor_stats = self._format_item_stats(item_info)
                if armor_stats:
                    stats_label = tk.Label(
                        item_frame,
                        text=f"📊 {armor_stats}",
        ...
        if not accessories_list:
            ...
            for item_name, quantity, item_info in accessories_list:
                ...
                accessory_stats = self._format_item_stats(item_info)
                if accessory_stats:
                    stats_label = tk.Label(
                        item_frame,
                        text=f"📊 {accessory_stats}",
    ...
    def use_item_dialog(self, parent):
        ...
        quantity_frame = tk.Frame(dialog, bg=self.colors['bg'])
        ...
        def on_use():
            ...
            if selection:
                ...
                quantity = usable_items[index][1]
                item_info = usable_items[index][2]

                try:
                    use_qty = int(quantity_var.get())
                    use_qty = min(quantity, max(1, use_qty))
                    
                    # 批量使用物品
                    for i in range(use_qty):
                        if not self.game.player.use_item(item_name):
                    ...
                    self.update_game_info()
                    messagebox.showinfo("成功", f"使用了 {use_qty} 个 {item_name}！")
                    dialog.destroy()
                except ValueError:
    ...
    def show_map(self):
        ...
        for dim_key, dim_info in dimensions.items():
            if dim_key in ['time', 'dream', 'shadow'] and not self.game.new_game_plus_unlocked:
                continue  # 直接跳过，不创建该维度的标签页
            ...
            for scene_key, scene_data in dim_info['scenes']:
                ...
                elif scene_key == "shadow_trial":
                    # 暗影试炼的特殊解锁条件
                    status_text = "🔒 需解锁: 70000金币 + 1把暗影密钥"
    ...
    def talk_to_npc(self, npc_name):
        ...
        if 'trades' in npc_data and npc_data['trades']:
            ...
            # 创建可滚动的购买列表
            buy_canvas = tk.Canvas(buy_frame, bg='#1e1e1e', highlightthickness=0)
            buy_scrollbar = tk.Scrollbar(buy_frame, orient='vertical', command=buy_canvas.yview)
            buy_scrollable = tk.Frame(buy_canvas, bg='#1e1e1e')
            ...
            for item_name, price in trade_items:
                ...
                # 物品名称和描述
                info_frame = tk.Frame(item_frame, bg='#1e1e1e')
                ...
                # 购买区域
                buy_action_frame = tk.Frame(item_frame, bg='#1e1e1e')
                buy_action_frame.pack(side='right')
                ...
                def make_buy_handler(qv, item, cost):
                    def handler():
                        try:
                            qty = int(qv.get())
                            qty = max(1, qty)
                            ...
                            if self.game.player.gold >= total_cost:
                                ...
                                self.game.player.add_item(item, qty, self.game)
                                self.update_game_info()
                                gold_label.config(text=f"💰 你的金币: {self.game.player.gold}")
                                messagebox.showinfo("成功", f"购买了 {qty} 个 {item}！")
                                dialog.destroy()
                                self.talk_to_npc(npc_name)
                            else:
                                messagebox.showerror("错误", f"金币不足！需要 {total_cost} 金币，当前只有 {self.game.player.gold} 金币。")
                        except ValueError:
                            messagebox.showerror("错误", "请输入有效的数量！")
                    return handler
                ...
                buy_btn = tk.Button(
                    buy_action_frame,
                    text="购买",
                    command=make_buy_handler(qty_var, item_name, price),
                    font=self.small_font,
            ...
            # 没有交易物品时显示提示
            no_trade_frame = tk.Frame(notebook, bg='#1e1e1e')
            notebook.add(no_trade_frame, text="🛒 购买")
            no_trade_label = tk.Label(
                no_trade_frame,
                text="该NPC没有商品出售",
                font=self.normal_font,
                fg=self.colors['fg'],
        ...
        # 创建可滚动的出售列表
        sell_canvas = tk.Canvas(sell_frame, bg='#1e1e1e', highlightthickness=0)
        sell_scrollbar = tk.Scrollbar(sell_frame, orient='vertical', command=sell_canvas.yview)
        sell_scrollable = tk.Frame(sell_canvas, bg='#1e1e1e')
        
        sell_scrollable.bind(
            "<Configure>",
            lambda e: sell_canvas.configure(scrollregion=sell_canvas.bbox("all"))
        )
        
        sell_canvas.create_window((0, 0), window=sell_scrollable, anchor="nw")
        sell_canvas.configure(yscrollcommand=sell_scrollbar.set)
        
        def on_sell_mouse_wheel(event):
            sell_canvas.yview_scroll(int(-1 * (event.delta / 120)), "units")
        sell_canvas.bind("<MouseWheel>", on_sell_mouse_wheel)
        
        # 收集可出售的物品（材料、宝藏、消耗品、矿石、锭等）
        sellable_items = []
        for item_name, quantity in self.game.player.inventory.items():
            if item_name in self.game.items:
                item_info = self.game.items[item_name]
                item_type = item_info.get('type', '')
                # 可出售类型：材料、宝藏、消耗品、矿石、锭、结晶、碎片等
                if item_type in ['material', 'treasure', 'consumable']:
                    # 排除一些特殊物品
                    if item_name not in ['暗影密钥']:
                        sellable_items.append((item_name, quantity, item_info.get('value', 10)))
                elif '矿石' in item_name or '锭' in item_name or '结晶' in item_name or '碎片' in item_name:
                    sellable_items.append((item_name, quantity, item_info.get('value', 10)))
                elif '精华' in item_name:
                    sellable_items.append((item_name, quantity, item_info.get('value', 50)))
        
        if sellable_items:
            for item_name, quantity, value in sellable_items:
                item_frame = tk.Frame(sell_scrollable, bg='#1e1e1e')
                item_frame.pack(fill='x', padx=5, pady=3)
                ...
                # 获取物品描述
                item_info = self.game.items.get(item_name, {})
                desc = item_info.get('description', '')
                desc_label = tk.Label(
                    info_frame,
                    text=f"  {desc}  (💰 {value} 金币/个)",
                    font=self.small_font,
                    fg=self.colors['fg'],
                    bg='#1e1e1e',
                    anchor='w'
                ...
                # 出售区域
                sell_action_frame = tk.Frame(item_frame, bg='#1e1e1e')
                sell_action_frame.pack(side='right')
                
                # 数量输入
                sell_qty_var = tk.StringVar(value="1")
                sell_qty_spinbox = tk.Spinbox(
                    sell_action_frame,
                    from_=1,
                    to=quantity,
                    textvariable=sell_qty_var,
                    width=4
                )
                sell_qty_spinbox.pack(side='left', padx=2)
                
                def make_sell_handler(qv, item, cost, max_qty):
                    def handler():
                        try:
                            qty = int(qv.get())
                            qty = min(max_qty, max(1, qty))
                            total = cost * qty
                ...
                sell_btn = tk.Button(
                    sell_action_frame,
                    text="出售",
                    command=make_sell_handler(sell_qty_var, item_name, value, quantity),
                    font=self.small_font,
    ...
    def trade_with_npc(self, npc_name, parent_dialog):
        ...
        for item_name, price in trade_items:
            ...
            buy_btn = tk.Button(
                item_frame,
                text="购买",
                command=make_buy_handler(quantity_var, item_name, price),
                font=self.small_font,
                bg=self.colors['button_bg'],
                fg=self.colors['button_fg']
            )
    ...
    def pause_game(self):
        ...
        settings_btn = tk.Button(
            button_frame,
            text="关于游戏",
            command=lambda: [dialog.destroy(), self.show_about()],
            font=self.normal_font,
            bg=self.colors['button_bg'],
            fg=self.colors['button_fg'],
            width=15,
...
class Game:
    ...
    def __init__(self, gui):
        ...
        #2.2.6缺失
        self.new_game_plus_unlocked = False
    ...
    def handle_player_death(self, reason="你死了"):
        """处理玩家死亡 - 究极模式永久死亡"""
        # 防止重复调用
        ...
        if self.config['difficulty'] == 'ultimate':
            self.add_message(f"💀 究极模式：{reason}！永久死亡！", 'error')
            self.add_message("🗑️ 存档将被删除...", 'error')
            
            # 删除存档
            if hasattr(self, 'current_save') and self.current_save:
                save_path = os.path.join(self.saves_dir, self.current_save)
                try:
                    if os.path.exists(save_path):
                        os.remove(save_path)
                        self.add_message("📁 存档已删除！", 'warning')
                except Exception as e:
                    print(f"删除存档失败: {e}")
            
            # 停止体力恢复线程
            self.stamina_recovery_running = False
            if self.stamina_recovery_thread and self.stamina_recovery_thread.is_alive():
                try:
                    self.stamina_recovery_thread.join(timeout=0.5)
                except:
                    pass
            ...
            def game_over():
                try:
                    # 关闭可能打开的战斗对话框
                    if hasattr(self, '_battle_dialog') and self._battle_dialog:
                        try:
                            self._battle_dialog.destroy()
                        except:
                            pass
                except:
                    pass
                ...
                messagebox.showinfo("💀 游戏结束", 
                    f"究极模式：{self.player.name if self.player else '冒险者'} 已永久死亡！\n"
                    f"等级: {self.player.level if self.player else 0}\n"
                    f"击败敌人: {self.enemies_defeated}\n"
                    f"存活天数: {self.day_count}\n\n"
                    "存档已被删除。")
                self.game_state = "menu"
                self.player = None  # 清空玩家防止进一步操作
                if self.gui:
            ...
            if self.gui:
                self.gui.root.after(100, game_over)
            else:
                game_over()
            return True
        else:
            ...
            if self.player:
                self.player.hp = 1
                self.add_message(f"💀 {reason}，你勉强活了下来。", 'error')
                if self.gui:
                    self.gui.update_game_info()
            return False
    ...
    def initialize_game_data(self):
        ...
        self.scenes = {
            ...
            "shadow_trial": {
                "name": "暗影试炼",
                "description": "暗影之地的最终试炼，只有真正的勇者才能通过。",
                "enemies": ["暗影君主"],
                "items": [],
                "npcs": ["暗影使者"],
                "events": [],
        ...
        self.enemies = {
            ...
            "暗影君主": {"hp": 1800, "attack": 160, "defense": 100, "exp": 8000, "gold": 8000, "drops": ["暗影皇冠"], "is_boss": True, "is_final_boss": True, "boss_theme": "最终Boss", "special_abilities": ["暗影之力", "黑暗冲击", "灵魂吞噬", "终极黑暗"]},
            # 缺失的敌人定义
        ...
        self.npcs = {
            ...
            "工程师": {"dialogue": "科技的力量是无限的，只要有足够的创造力和资源。", "trades": {"机械零件": 100, "能量核心": 180, "机械助手": 400, "奥金矿石": 800}},
            "科学家": {"dialogue": "知识是最强大的力量，通过研究，我们可以理解这个世界的奥秘。", "trades": {"科学仪器": 200, "研究笔记": 150, "实验材料": 100, "星铁矿石": 1200}},
            ...
            "矮人矿工": {"dialogue": "挖矿是我们矮人的传统职业，我们在地下深处寻找珍贵的矿石。", "trades": {"矿石": 20, "秘银矿石": 80,"星铁矿石": 1200}},
            ...
            "创世神": {"dialogue": "我是世界的创造者，见证了无数的轮回和变迁。", "trades": {"神之祝福": 500, "世界之心": 1000,"创世宝石碎片": 2000}},
        ...
        self.items = {
            ...
            # ---- 锭类物品（锻造材料） ----
            "铜锭": {"type": "material", "effect": "craft", "value": 40, "description": "铜矿石熔炼而成的铜锭，可用于锻造基础装备", "durability": 0},
            "铁锭": {"type": "material", "effect": "craft", "value": 80, "description": "铁矿石熔炼而成的铁锭，可用于锻造中级装备", "durability": 0},
            "银锭": {"type": "material", "effect": "craft", "value": 130, "description": "银矿石熔炼而成的银锭，可用于锻造高级装备", "durability": 0},
            "金锭": {"type": "material", "effect": "craft", "value": 250, "description": "金矿石熔炼而成的金锭，可用于锻造稀有装备", "durability": 0},
            "秘银锭": {"type": "material", "effect": "craft", "value": 380, "description": "秘银矿石熔炼而成的秘银锭，可用于锻造史诗装备", "durability": 0},
            "精金锭": {"type": "material", "effect": "craft", "value": 600, "description": "精金矿石熔炼而成的精金锭，可用于锻造传说装备", "durability": 0},
            "奥金锭": {"type": "material", "effect": "craft", "value": 950, "description": "奥金矿石熔炼而成的奥金锭，可用于锻造神话装备", "durability": 0},
            "星铁锭": {"type": "material", "effect": "craft", "value": 1400, "description": "星铁矿石熔炼而成的星铁锭，可用于锻造创世装备", "durability": 0},
            "矿石锭": {"type": "material", "effect": "craft", "value": 50, "description": "普通矿石熔炼而成的锭，基础锻造材料", "durability": 0},
            
            # ---- 结晶类物品（高级材料） ----
            "火焰结晶": {"type": "material", "effect": "craft", "value": 180, "description": "火焰精华凝聚而成的结晶，蕴含强大的火元素能量", "durability": 0},
            "冰霜结晶": {"type": "material", "effect": "craft", "value": 180, "description": "冰霜精华凝聚而成的结晶，蕴含强大的冰元素能量", "durability": 0},
            "雷电结晶": {"type": "material", "effect": "craft", "value": 180, "description": "雷电精华凝聚而成的结晶，蕴含强大的雷元素能量", "durability": 0},
            "自然结晶": {"type": "material", "effect": "craft", "value": 180, "description": "自然精华凝聚而成的结晶，蕴含强大的自然能量", "durability": 0},
            "暗影结晶": {"type": "material", "effect": "craft", "value": 220, "description": "暗影精华凝聚而成的结晶，蕴含强大的暗影能量", "durability": 0},
            "光明结晶": {"type": "material", "effect": "craft", "value": 220, "description": "光明精华凝聚而成的结晶，蕴含强大的光明能量", "durability": 0},
            
            # ---- 其他合成材料 ----
            "木板": {"type": "material", "effect": "craft", "value": 15, "description": "木材加工而成的木板，可用于制作基础装备", "durability": 0},
            "木炭": {"type": "material", "effect": "craft", "value": 10, "description": "木材烧制而成的木炭，可用于锻造和炼金", "durability": 0},
            "玻璃瓶": {"type": "material", "effect": "craft", "value": 15, "description": "玻璃制成的瓶子，可用于装药水", "durability": 0},
            "丝线": {"type": "material", "effect": "craft", "value": 25, "description": "蛛丝加工而成的丝线，可用于制作布甲和饰品", "durability": 0},
            
            # ---- 龙类合成物品 ----
            "龙血药水": {"type": "consumable", "effect": "heal", "value": 300, "description": "蕴含龙血的强大药水，恢复300点生命值", "durability": 1},
            "龙心药剂": {"type": "consumable", "effect": "heal", "value": 9999, "description": "以龙心炼制的传奇药剂，完全恢复生命值", "durability": 1},
            "远古龙鳞甲": {"type": "armor", "effect": "defense", "value": 450, "description": "远古龙鳞打造的传说级铠甲，增加150点防御力和100点生命值", "durability": 400,
                          "bonus_defense": 150, "bonus_hp": 100, "bonus_fire_resist": 60, "bonus_magic_resist": 30},
            # ========== 武器系统 ==========
            # 基础武器
            ...
            "暗影皇冠": {"type": "accessory", "effect": "special", "value": 5000, "description": "暗影君主的皇冠，装备后为各项属性大大加成", "durability": 500, "bonus_attack": 200, "bonus_defense": 150, "bonus_hp": 500, "bonus_magic": 100, "bonus_speed": 50, "bonus_crit": 30, "bonus_dodge": 20, "bonus_gold": 50, "bonus_exp": 50},

            # ========== 敌人掉落物 - 已为每件战利品赋予真实功能属性 ==========
            # ---- 武器（可装备，提升攻击力） ----
            "亡灵剑": {"type": "weapon", "effect": "attack", "value": 80, "description": "亡灵战士遗留的剑，附有死灵之力", "durability": 100, "bonus_attack": 35, "bonus_magic": 10},
            "元素之剑": {"type": "weapon", "effect": "attack", "value": 220, "description": "蕴含元素之力的剑，攻击与法术兼备", "durability": 180, "bonus_attack": 80, "bonus_magic": 30},
            "光明之剑": {"type": "weapon", "effect": "attack", "value": 200, "description": "散发圣光的剑，对黑暗系敌人造成额外伤害", "durability": 180, "bonus_attack": 75, "bonus_against_dark": 60},
            "冥界之剑": {"type": "weapon", "effect": "attack", "value": 130, "description": "冥界守卫的佩剑，附带幽冥之力", "durability": 120, "bonus_attack": 55, "bonus_magic": 15},
            "创世神剑": {"type": "weapon", "effect": "attack", "value": 1500, "description": "创世神遗留的至高之剑，拥有改写世界的力量", "durability": 500, "bonus_attack": 250, "bonus_magic": 100, "bonus_crit": 25},
            "天使之剑": {"type": "weapon", "effect": "attack", "value": 160, "description": "天使战士的圣剑，光耀夺目", "durability": 150, "bonus_attack": 65, "bonus_magic": 20},
            "天文之剑": {"type": "weapon", "effect": "attack", "value": 150, "description": "刻有星图的剑，引动星辰之力", "durability": 150, "bonus_attack": 60, "bonus_magic": 15},
            "天空之剑": {"type": "weapon", "effect": "attack", "value": 130, "description": "天空守卫的轻盈之剑，挥舞如风", "durability": 130, "bonus_attack": 55, "bonus_speed": 5},
            "守卫之剑": {"type": "weapon", "effect": "attack", "value": 90, "description": "守卫的标准佩剑，攻守均衡", "durability": 100, "bonus_attack": 45, "bonus_defense": 5},
            "守卫剑": {"type": "weapon", "effect": "attack", "value": 70, "description": "普通守卫的制式长剑", "durability": 80, "bonus_attack": 35},
            "守护者之剑": {"type": "weapon", "effect": "attack", "value": 220, "description": "守护者的重剑，攻防俱佳", "durability": 180, "bonus_attack": 85, "bonus_defense": 15},
            "弯刀": {"type": "weapon", "effect": "attack", "value": 45, "description": "游牧强盗的弯刀，锋利轻巧", "durability": 60, "bonus_attack": 22},
            "埃及弯刀": {"type": "weapon", "effect": "attack", "value": 85, "description": "古埃及士兵的弯刀，造型独特", "durability": 100, "bonus_attack": 40, "bonus_gold": 5},
            "恶魔之剑": {"type": "weapon", "effect": "attack", "value": 200, "description": "恶魔卫士的邪剑，带有吸血效果", "durability": 180, "bonus_attack": 80, "lifesteal": 8},
            "时间之剑": {"type": "weapon", "effect": "attack", "value": 280, "description": "掌控时间流逝的神秘之剑", "durability": 200, "bonus_attack": 95, "bonus_speed": 15},
            "暗影匕首": {"type": "weapon", "effect": "attack", "value": 100, "description": "暗影刺客的短匕，暴击极高", "durability": 120, "bonus_attack": 40, "bonus_crit": 15, "bonus_speed": 10},
            "水晶剑": {"type": "weapon", "effect": "attack", "value": 110, "description": "水晶守卫的水晶之剑，晶莹剔透", "durability": 130, "bonus_attack": 48, "bonus_magic": 12},
            "海盗剑": {"type": "weapon", "effect": "attack", "value": 95, "description": "海盗船长的佩剑，掠夺成性", "durability": 110, "bonus_attack": 42, "bonus_gold": 10},
            "海盗匕首": {"type": "weapon", "effect": "attack", "value": 75, "description": "飞行海盗的匕首，轻便致命", "durability": 100, "bonus_attack": 35, "bonus_crit": 12},
            "深渊之剑": {"type": "weapon", "effect": "attack", "value": 320, "description": "深渊领主的邪剑，吞噬生命", "durability": 250, "bonus_attack": 110, "bonus_magic": 30, "lifesteal": 10},
            "激光枪": {"type": "weapon", "effect": "attack", "value": 260, "description": "太空海盗的高科技武器，发射激光", "durability": 200, "bonus_attack": 90, "bonus_magic": 25},
            "矮人战斧": {"type": "weapon", "effect": "attack", "value": 110, "description": "矮人守卫的重型战斧，势大力沉", "durability": 150, "bonus_attack": 50, "bonus_defense": 8},
            "巨人战斧": {"type": "weapon", "effect": "attack", "value": 180, "description": "冰霜巨人遗留的巨型战斧", "durability": 180, "bonus_attack": 75, "bonus_defense": 10},
            "神圣之剑": {"type": "weapon", "effect": "attack", "value": 220, "description": "神之守卫的圣剑，克制黑暗", "durability": 200, "bonus_attack": 85, "bonus_against_dark": 80},
            "精灵之剑": {"type": "weapon", "effect": "attack", "value": 130, "description": "精灵战士的轻盈长剑", "durability": 140, "bonus_attack": 55, "bonus_magic": 15, "bonus_speed": 5},
            "风暴之剑": {"type": "weapon", "effect": "attack", "value": 240, "description": "风暴领主的佩剑，可召唤连锁闪电", "durability": 200, "bonus_attack": 90, "chain_lightning": True},
            "黑暗之剑": {"type": "weapon", "effect": "attack", "value": 180, "description": "黑暗骑士的邪剑，吸收敌人生命", "durability": 180, "bonus_attack": 75, "lifesteal": 10},
            "骨杖": {"type": "weapon", "effect": "attack", "value": 90, "description": "亡灵法师的骨制法杖，增强魔法", "durability": 100, "bonus_attack": 15, "bonus_magic": 40},
            "收割者镰刀": {"type": "weapon", "effect": "attack", "value": 150, "description": "灵魂收割者的镰刀，收割生命", "durability": 150, "bonus_attack": 65, "lifesteal": 12},
            "忍者镖": {"type": "weapon", "effect": "attack", "value": 70, "description": "忍者的飞镖，可远程投掷", "durability": 80, "bonus_attack": 30, "bonus_crit": 10, "bonus_speed": 8},
            "破旧武器": {"type": "weapon", "effect": "attack", "value": 20, "description": "掠夺者丢弃的破旧武器，聊胜于无", "durability": 30, "bonus_attack": 10},
            # ---- 护甲（可装备，提升防御力） ----
            "卫士盔甲": {"type": "armor", "effect": "defense", "value": 180, "description": "恶魔卫士的重型盔甲，坚不可摧", "durability": 200, "bonus_defense": 60, "bonus_hp": 50},
            "刺客斗篷": {"type": "armor", "effect": "defense", "value": 90, "description": "暗影刺客的斗篷，提升闪避与速度", "durability": 100, "bonus_defense": 20, "bonus_dodge": 15, "bonus_speed": 10},
            "守卫之盾": {"type": "armor", "effect": "defense", "value": 100, "description": "知识守卫的盾牌，坚固耐用", "durability": 120, "bonus_defense": 35, "bonus_hp": 20},
            "守卫护盾": {"type": "armor", "effect": "defense", "value": 110, "description": "水晶守卫的护盾，散发晶光", "durability": 130, "bonus_defense": 38, "bonus_hp": 25},
            "守卫盔甲": {"type": "armor", "effect": "defense", "value": 90, "description": "标准守卫盔甲，防护良好", "durability": 100, "bonus_defense": 30, "bonus_hp": 15},
            "守护者之盾": {"type": "armor", "effect": "defense", "value": 220, "description": "梦境守护者的重盾，坚若磐石", "durability": 200, "bonus_defense": 70, "bonus_hp": 80},
            "战士盔甲": {"type": "armor", "effect": "defense", "value": 130, "description": "战士的制式盔甲", "durability": 150, "bonus_defense": 45, "bonus_hp": 30},
            "忍者服": {"type": "armor", "effect": "defense", "value": 80, "description": "忍者的紧身服，灵活轻便", "durability": 100, "bonus_defense": 20, "bonus_dodge": 20, "bonus_speed": 10},
            "和服": {"type": "armor", "effect": "defense", "value": 90, "description": "武士的和服，附有轻微法力", "durability": 100, "bonus_defense": 22, "bonus_magic": 10},
            "太空头盔": {"type": "armor", "effect": "defense", "value": 120, "description": "太空海盗的头盔，抵御辐射", "durability": 120, "bonus_defense": 30, "bonus_magic_resist": 15},
            "强盗头巾": {"type": "armor", "effect": "defense", "value": 35, "description": "强盗的头巾，遮掩身份", "durability": 50, "bonus_defense": 10, "bonus_dodge": 5},
            "暗影长袍": {"type": "armor", "effect": "defense", "value": 110, "description": "暗影法师的长袍，增强魔法", "durability": 120, "bonus_defense": 25, "bonus_magic": 20},
            "熔岩护盾": {"type": "armor", "effect": "defense", "value": 160, "description": "熔岩守卫的护盾，抵御火焰", "durability": 180, "bonus_defense": 55, "bonus_fire_resist": 40},
            "神圣护盾": {"type": "armor", "effect": "defense", "value": 200, "description": "天使守卫的圣盾，神圣庇护", "durability": 200, "bonus_defense": 65, "bonus_hp": 60, "bonus_magic_resist": 20},
            "神圣盔甲": {"type": "armor", "effect": "defense", "value": 190, "description": "天使战士的神圣盔甲", "durability": 200, "bonus_defense": 60, "bonus_hp": 70},
            "自然之盾": {"type": "armor", "effect": "defense", "value": 150, "description": "自然守卫的藤木之盾", "durability": 180, "bonus_defense": 50, "bonus_hp": 40},
            "骑士盔甲": {"type": "armor", "effect": "defense", "value": 140, "description": "黑暗骑士的重型盔甲", "durability": 160, "bonus_defense": 48, "bonus_hp": 35},
            "防毒面具": {"type": "armor", "effect": "defense", "value": 70, "description": "毒气怪的防毒面具，免疫毒气", "durability": 80, "bonus_defense": 15, "bonus_magic_resist": 15},
            "牛角盔": {"type": "armor", "effect": "defense", "value": 85, "description": "维京战士的牛角盔，威武雄壮", "durability": 100, "bonus_defense": 25, "bonus_hp": 15},
            "将军头盔": {"type": "armor", "effect": "defense", "value": 130, "description": "幕府将军的头盔，气势逼人", "durability": 150, "bonus_defense": 40, "bonus_hp": 25},
            "掠夺者头盔": {"type": "armor", "effect": "defense", "value": 90, "description": "掠夺者的头盔，防护头部", "durability": 100, "bonus_defense": 28, "bonus_hp": 15},
            "神之护盾": {"type": "armor", "effect": "defense", "value": 300, "description": "神之守卫的圣盾，神圣不可侵犯", "durability": 250, "bonus_defense": 90, "bonus_hp": 100, "bonus_magic_resist": 30},
            "船长帽": {"type": "armor", "effect": "defense", "value": 90, "description": "海盗船长的帽子，彰显身份", "durability": 100, "bonus_defense": 18, "bonus_gold": 10},
            "飞行装备": {"type": "armor", "effect": "defense", "value": 100, "description": "飞行海盗的装备，便于空中作战", "durability": 110, "bonus_defense": 25, "bonus_speed": 10},
            # ---- 饰品（可装备，多种属性加成） ----
            "君主王冠": {"type": "accessory", "effect": "special", "value": 400, "description": "元素君主的王冠，全属性提升", "durability": 150, "bonus_attack": 40, "bonus_defense": 30, "bonus_hp": 100},
            "噩梦王冠": {"type": "accessory", "effect": "special", "value": 350, "description": "噩梦领主的王冠，增强法力", "durability": 150, "bonus_attack": 30, "bonus_magic": 40},
            "黑暗王冠": {"type": "accessory", "effect": "special", "value": 380, "description": "黑暗领主的王冠，附带吸血", "durability": 150, "bonus_attack": 35, "bonus_defense": 25, "lifesteal": 5},
            "领主王冠": {"type": "accessory", "effect": "special", "value": 420, "description": "深渊/风暴领主的王冠，威严无比", "durability": 160, "bonus_attack": 45, "bonus_defense": 35, "bonus_hp": 120},
            "梦境主宰之冠": {"type": "accessory", "effect": "special", "value": 1500, "description": "梦境主宰的至高王冠，掌控梦境与现实", "durability": 300, "bonus_attack": 100, "bonus_defense": 80, "bonus_hp": 300, "bonus_magic": 80},
            "守卫徽章": {"type": "accessory", "effect": "defense", "value": 60, "description": "守卫的徽章，象征职责", "durability": 80, "bonus_defense": 15},
            "巨头徽章": {"type": "accessory", "effect": "special", "value": 150, "description": "工业巨头的徽章，增加金币获取", "durability": 100, "bonus_gold": 15, "bonus_defense": 10},
            "时间领主徽章": {"type": "accessory", "effect": "special", "value": 500, "description": "时间领主的徽章，提升速度与经验", "durability": 150, "bonus_attack": 30, "bonus_speed": 20, "bonus_exp": 30},
            "挑战徽章": {"type": "accessory", "effect": "attack", "value": 120, "description": "英雄挑战的徽章，提升暴击", "durability": 100, "bonus_attack": 20, "bonus_crit": 5},
            "冒险徽章": {"type": "accessory", "effect": "special", "value": 100, "description": "冒险怪物的徽章，增加经验与幸运", "durability": 100, "bonus_exp": 15, "bonus_luck": 10},
            "勇气徽章": {"type": "accessory", "effect": "attack", "value": 130, "description": "恐惧怪兽掉落的勇气徽章", "durability": 100, "bonus_attack": 22, "bonus_defense": 10},
            "守卫沙漏": {"type": "accessory", "effect": "special", "value": 150, "description": "时间守卫的沙漏，提升速度", "durability": 100, "bonus_speed": 10, "bonus_defense": 10},
            "时间沙漏": {"type": "accessory", "effect": "special", "value": 300, "description": "时间守护者的沙漏，掌控时间流速", "durability": 150, "bonus_speed": 25, "bonus_exp": 20},
            "火焰宝珠": {"type": "accessory", "effect": "magic", "value": 200, "description": "远古火龙的宝珠，蕴含火焰之力", "durability": 120, "bonus_magic": 30, "element": "fire"},
            "现实之石": {"type": "accessory", "effect": "special", "value": 250, "description": "幻想怪物掉落的现实之石，稳定现实", "durability": 120, "bonus_defense": 20, "bonus_magic_resist": 15},
            "光明之石": {"type": "accessory", "effect": "special", "value": 150, "description": "黑暗阴影中孕育的光明之石", "durability": 100, "bonus_hp": 50, "bonus_heal": 10},
            "纯真守护符": {"type": "accessory", "effect": "special", "value": 180, "description": "纯真破坏者掉落的守护符，护佑纯真", "durability": 120, "bonus_defense": 15, "bonus_hp": 60},
            "魔晶石": {"type": "accessory", "effect": "magic", "value": 100, "description": "石魔体内的魔晶石，增强魔法", "durability": 100, "bonus_magic": 25},
            "亡灵法术书": {"type": "accessory", "effect": "magic", "value": 150, "description": "亡灵法师的法术书，记载死灵魔法", "durability": 120, "bonus_magic": 35},
            "幽灵之书": {"type": "accessory", "effect": "magic", "value": 120, "description": "智慧幽灵遗留的魔法之书", "durability": 100, "bonus_magic": 28},
            "黑暗法术书": {"type": "accessory", "effect": "magic", "value": 180, "description": "暗影法师的黑暗法术书", "durability": 120, "bonus_magic": 40, "lifesteal": 3},
            "智慧之书": {"type": "accessory", "effect": "magic", "value": 200, "description": "知识守卫的智慧之书，大幅增强法力", "durability": 150, "bonus_magic": 45, "bonus_exp": 10},
            # ---- 宝石（可镶嵌） ----
            "龙晶": {"type": "gem", "effect": "attack", "value": 200, "description": "水晶龙体内的龙晶，增加攻击与暴击", "durability": 0, "bonus_attack": 25, "bonus_crit": 10},
            "龙焰宝石": {"type": "gem", "effect": "attack", "value": 250, "description": "火山龙/巨龙掉落的龙焰宝石，蕴含龙焰", "durability": 0, "bonus_attack": 30, "element": "fire"},
            # ---- 消耗品（可使用） ----
            "夜视药水": {"type": "consumable", "effect": "exp", "value": 30, "description": "蝙蝠掉落的药水，使用后获得30点探索经验", "durability": 1},
            "魔法草药": {"type": "consumable", "effect": "heal", "value": 35, "description": "森林精灵的魔法草药，恢复35点生命值", "durability": 1},
            "蛙腿": {"type": "consumable", "effect": "heal", "value": 15, "description": "毒蛙的腿，食用恢复15点生命值", "durability": 1},
            "飞行肉": {"type": "consumable", "effect": "heal", "value": 20, "description": "飞行野兽的肉，食用恢复20点生命值", "durability": 1},
            "历史记录": {"type": "consumable", "effect": "exp", "value": 100, "description": "历史幽灵的记录，使用后获得100点经验值", "durability": 1},
            "科学笔记": {"type": "consumable", "effect": "exp", "value": 150, "description": "疯狂科学家的笔记，使用后获得150点经验值", "durability": 1},
            "童年阴影": {"type": "consumable", "effect": "buff_defense", "value": 15, "description": "童年恐惧的阴影，使用后防御力永久增加15点", "durability": 1},
            "法老诅咒": {"type": "consumable", "effect": "buff_attack", "value": 20, "description": "木乃伊的诅咒，使用后攻击力永久增加20点", "durability": 1},
            "恶魔契约": {"type": "consumable", "effect": "buff_attack", "value": 50, "description": "深渊领主的契约，使用后攻击力永久增加50点", "durability": 1},
            "黑暗之心": {"type": "consumable", "effect": "buff_attack", "value": 40, "description": "深渊领主的黑暗之心，使用后攻击力永久增加40点", "durability": 1},
            "成神之证": {"type": "consumable", "effect": "buff_attack", "value": 50, "description": "创世神遗留的成神凭证，使用后攻击力永久增加50点", "durability": 1},
            "龙之心": {"type": "consumable", "effect": "heal", "value": 9999, "description": "远古火龙的龙心，使用后生命值完全恢复", "durability": 1},
            "龙之祝福": {"type": "consumable", "effect": "heal", "value": 500, "description": "远古火龙的祝福，使用后恢复500点生命值", "durability": 1},
            # ---- 宝藏（可使用，换取金币或属性） ----
            "偷来的钱包": {"type": "treasure", "effect": "sell", "value": 50, "description": "小偷偷来的钱包，可出售换取金币", "durability": 0},
            "假金币": {"type": "treasure", "effect": "sell", "value": 30, "description": "骗子伪造的金币，仍可出售换些金币", "durability": 0},
            "欺诈手册": {"type": "treasure", "effect": "sell", "value": 80, "description": "骗子的欺诈手册，可出售换取金币", "durability": 0},
            "工厂钥匙": {"type": "treasure", "effect": "sell", "value": 80, "description": "工厂守卫的钥匙，可出售换取金币", "durability": 0},
            "迷宫钥匙": {"type": "treasure", "effect": "sell", "value": 100, "description": "迷宫守卫的钥匙，可出售换取金币", "durability": 0},
            "管家钥匙": {"type": "treasure", "effect": "sell", "value": 60, "description": "花园管家的钥匙，可出售换取金币", "durability": 0},
            "法老硬币": {"type": "treasure", "effect": "sell", "value": 200, "description": "埃及士兵的硬币，可出售换取大量金币", "durability": 0},
            "幕府令牌": {"type": "treasure", "effect": "sell", "value": 200, "description": "幕府将军的令牌，可出售换取大量金币", "durability": 0},
            # ---- 材料（用于合成与交易） ----
            "云之精华": {"type": "material", "effect": "craft", "value": 100, "description": "云精灵的精华，可用于合成天空系物品", "durability": 0},
            "元素精华": {"type": "material", "effect": "craft", "value": 100, "description": "元素精灵的精华，可用于合成元素装备", "durability": 0},
            "冰之精华": {"type": "material", "effect": "craft", "value": 100, "description": "冰元素的精华，可用于合成冰霜装备", "durability": 0},
            "嫉妒精华": {"type": "material", "effect": "craft", "value": 80, "description": "嫉妒恶魔的精华", "durability": 0},
            "孤独精华": {"type": "material", "effect": "craft", "value": 80, "description": "孤独幽灵的精华", "durability": 0},
            "悲伤精华": {"type": "material", "effect": "craft", "value": 80, "description": "悲伤阴影/精灵的精华", "durability": 0},
            "智慧精华": {"type": "material", "effect": "craft", "value": 120, "description": "智慧幽灵的精华，可用于合成魔法物品", "durability": 0},
            "星之精华": {"type": "material", "effect": "craft", "value": 150, "description": "星灵的精华，蕴含星辰之力", "durability": 0},
            "神秘精华": {"type": "material", "effect": "craft", "value": 130, "description": "神秘生物的精华", "durability": 0},
            "雷之精华": {"type": "material", "effect": "craft", "value": 140, "description": "雷元素的精华，可用于合成雷电装备", "durability": 0},
            "风之精华": {"type": "material", "effect": "craft", "value": 120, "description": "风元素的精华", "durability": 0},
            "魔法精华": {"type": "material", "effect": "craft", "value": 120, "description": "魔法生物的精华", "durability": 0},
            "黑暗精华": {"type": "material", "effect": "craft", "value": 150, "description": "食尸鬼/黑暗守护者的精华", "durability": 0},
            "辐射精华": {"type": "material", "effect": "craft", "value": 100, "description": "变异生物的辐射精华", "durability": 0},
            "花之精华": {"type": "material", "effect": "craft", "value": 80, "description": "花精灵的精华", "durability": 0},
            "裂缝精华": {"type": "material", "effect": "craft", "value": 130, "description": "时空裂缝的精华", "durability": 0},
            "阴影精华": {"type": "material", "effect": "craft", "value": 100, "description": "黑暗阴影的精华", "durability": 0},
            "幽灵精华": {"type": "material", "effect": "craft", "value": 90, "description": "幽灵的精华，可用于合成灵魂物品", "durability": 0},
            "邪恶精华": {"type": "material", "effect": "craft", "value": 130, "description": "邪恶势力的精华", "durability": 0},
            "侵略者碎片": {"type": "material", "effect": "craft", "value": 120, "description": "宇宙侵略者的碎片", "durability": 0},
            "冰晶碎片": {"type": "material", "effect": "craft", "value": 60, "description": "冰元素的碎片，可用于合成冰霜物品", "durability": 0},
            "恶梦碎片": {"type": "material", "effect": "craft", "value": 120, "description": "恶梦生物的碎片", "durability": 0},
            "扭曲碎片": {"type": "material", "effect": "craft", "value": 200, "description": "现实扭曲者的碎片", "durability": 0},
            "时空碎片": {"type": "material", "effect": "craft", "value": 180, "description": "时空怪物/裂缝的碎片", "durability": 0},
            "星际碎片": {"type": "material", "effect": "craft", "value": 150, "description": "星际怪物的碎片", "durability": 0},
            "幻想碎片": {"type": "material", "effect": "craft", "value": 100, "description": "幻想怪物的碎片", "durability": 0},
            "阴影碎片": {"type": "material", "effect": "craft", "value": 80, "description": "悲伤阴影的碎片", "durability": 0},
            "破坏者碎片": {"type": "material", "effect": "craft", "value": 130, "description": "纯真破坏者的碎片", "durability": 0},
            "烦恼碎片": {"type": "material", "effect": "craft", "value": 70, "description": "成长烦恼的碎片", "durability": 0},
            "黑洞碎片": {"type": "material", "effect": "craft", "value": 300, "description": "黑洞守护者的碎片，蕴含空间之力", "durability": 0},
            "未知碎片": {"type": "material", "effect": "craft", "value": 100, "description": "未知怪物的碎片", "durability": 0},
            "神话碎片": {"type": "material", "effect": "craft", "value": 130, "description": "北欧神话生物的碎片", "durability": 0},
            "古代盔甲碎片": {"type": "material", "effect": "craft", "value": 80, "description": "古代守卫盔甲的碎片", "durability": 0},
            "灼热碎片": {"type": "material", "effect": "craft", "value": 90, "description": "火元素的灼热碎片", "durability": 0},
            "AI核心": {"type": "material", "effect": "craft", "value": 250, "description": "AI主宰的核心，蕴含智能", "durability": 0},
            "元素核心": {"type": "material", "effect": "craft", "value": 200, "description": "元素君主的核心", "durability": 0},
            "守卫核心": {"type": "material", "effect": "craft", "value": 100, "description": "机械守卫的核心", "durability": 0},
            "岩石核心": {"type": "material", "effect": "craft", "value": 90, "description": "岩石傀儡的核心", "durability": 0},
            "工业核心": {"type": "material", "effect": "craft", "value": 150, "description": "工业巨头的核心", "durability": 0},
            "怪兽核心": {"type": "material", "effect": "craft", "value": 120, "description": "机械怪兽的核心", "durability": 0},
            "怪物核心": {"type": "material", "effect": "craft", "value": 130, "description": "时空怪物的核心", "durability": 0},
            "恐惧核心": {"type": "material", "effect": "craft", "value": 150, "description": "恐惧怪兽的核心", "durability": 0},
            "毒气核心": {"type": "material", "effect": "craft", "value": 100, "description": "毒气怪的核心", "durability": 0},
            "熔岩核心": {"type": "material", "effect": "craft", "value": 180, "description": "熔岩怪的核心", "durability": 0},
            "火焰核心": {"type": "material", "effect": "craft", "value": 200, "description": "火焰巨兽的核心", "durability": 0},
            "森林核心": {"type": "material", "effect": "craft", "value": 160, "description": "森林巨兽的核心", "durability": 0},
            "梦境核心": {"type": "material", "effect": "craft", "value": 250, "description": "梦境巨龙的核心", "durability": 0},
            "魔法核心": {"type": "material", "effect": "craft", "value": 150, "description": "魔法构装体的核心", "durability": 0},
            "暗影核心": {"type": "material", "effect": "craft", "value": 160, "description": "暗影巨兽的核心", "durability": 0},
            "光明结晶": {"type": "material", "effect": "craft", "value": 150, "description": "黑暗守护者孕育的光明结晶", "durability": 0},
            "奇幻结晶": {"type": "material", "effect": "craft", "value": 120, "description": "魔法生物的奇幻结晶", "durability": 0},
            "宇宙结晶": {"type": "material", "effect": "craft", "value": 250, "description": "星际怪物的宇宙结晶", "durability": 0},
            "友谊结晶": {"type": "material", "effect": "craft", "value": 100, "description": "孤独幽灵的友谊结晶", "durability": 0},
            "快乐结晶": {"type": "material", "effect": "craft", "value": 100, "description": "悲伤精灵的快乐结晶", "durability": 0},
            "勇气结晶": {"type": "material", "effect": "craft", "value": 120, "description": "冒险怪物的勇气结晶", "durability": 0},
            "恐惧结晶": {"type": "material", "effect": "craft", "value": 130, "description": "噩梦领主的恐惧结晶", "durability": 0},
            "成熟结晶": {"type": "material", "effect": "craft", "value": 100, "description": "成长烦恼的成熟结晶", "durability": 0},
            "现实结晶": {"type": "material", "effect": "craft", "value": 200, "description": "现实扭曲者的结晶", "durability": 0},
            "真相结晶": {"type": "material", "effect": "craft", "value": 150, "description": "神秘生物的真相结晶", "durability": 0},
            "探索结晶": {"type": "material", "effect": "craft", "value": 120, "description": "未知怪物的探索结晶", "durability": 0},
            "梦境结晶": {"type": "material", "effect": "craft", "value": 180, "description": "梦境守护者的结晶", "durability": 0},
            "英雄结晶": {"type": "material", "effect": "craft", "value": 150, "description": "英雄挑战的结晶", "durability": 0},
            "爱心结晶": {"type": "material", "effect": "craft", "value": 100, "description": "嫉妒恶魔的爱心结晶", "durability": 0},
            "纯真结晶": {"type": "material", "effect": "craft", "value": 120, "description": "童年恐惧的纯真结晶", "durability": 0},
            "防御结晶": {"type": "material", "effect": "craft", "value": 150, "description": "宇宙侵略者的防御结晶", "durability": 0},
            "空间结晶": {"type": "material", "effect": "craft", "value": 200, "description": "黑洞守护者的空间结晶", "durability": 0},
            "冰霜水晶": {"type": "material", "effect": "craft", "value": 120, "description": "冰霜巨人的水晶，蕴含冰霜之力", "durability": 0},
            "岛屿水晶": {"type": "material", "effect": "craft", "value": 90, "description": "岛守卫的水晶", "durability": 0},
            "空气水晶": {"type": "material", "effect": "craft", "value": 100, "description": "风元素的空气水晶", "durability": 0},
            "闪电水晶": {"type": "material", "effect": "craft", "value": 130, "description": "雷元素的闪电水晶", "durability": 0},
            "熊皮": {"type": "material", "effect": "craft", "value": 40, "description": "巨熊的皮，可用于制作护甲", "durability": 0},
            "雪怪毛皮": {"type": "material", "effect": "craft", "value": 80, "description": "雪怪的毛皮，保暖坚韧", "durability": 0},
            "野兽毛皮": {"type": "material", "effect": "craft", "value": 50, "description": "史前野兽的毛皮", "durability": 0},
            "马皮": {"type": "material", "effect": "craft", "value": 25, "description": "野马的皮", "durability": 0},
            "蛇皮": {"type": "material", "effect": "craft", "value": 35, "description": "沼泽蛇/沙漠巨蛇的皮", "durability": 0},
            "狼皮": {"type": "material", "effect": "craft", "value": 25, "description": "野狼的皮", "durability": 0},
            "蝙蝠翼": {"type": "material", "effect": "craft", "value": 15, "description": "蝙蝠的翅膀", "durability": 0},
            "野兽羽毛": {"type": "material", "effect": "craft", "value": 40, "description": "飞行野兽的羽毛", "durability": 0},
            "精灵羽毛": {"type": "material", "effect": "craft", "value": 60, "description": "云精灵的羽毛", "durability": 0},
            "轻盈羽毛": {"type": "material", "effect": "craft", "value": 50, "description": "风元素的轻盈羽毛", "durability": 0},
            "飞行翼": {"type": "material", "effect": "craft", "value": 70, "description": "飞行怪物的翅膀", "durability": 0},
            "巨兽利爪": {"type": "material", "effect": "craft", "value": 100, "description": "暗影/森林巨兽的利爪", "durability": 0},
            "巨兽牙齿": {"type": "material", "effect": "craft", "value": 90, "description": "火焰巨兽的牙齿", "durability": 0},
            "怪物牙齿": {"type": "material", "effect": "craft", "value": 60, "description": "飞行怪物的牙齿", "durability": 0},
            "巨型牙齿": {"type": "material", "effect": "craft", "value": 70, "description": "雪怪的巨型牙齿", "durability": 0},
            "兽牙": {"type": "material", "effect": "craft", "value": 30, "description": "史前野兽的兽牙", "durability": 0},
            "兽骨": {"type": "material", "effect": "craft", "value": 25, "description": "原始人的兽骨", "durability": 0},
            "巨人之骨": {"type": "material", "effect": "craft", "value": 100, "description": "古代巨人的骨头", "durability": 0},
            "地狱犬牙": {"type": "material", "effect": "craft", "value": 120, "description": "地狱犬的利牙", "durability": 0},
            "食尸鬼爪": {"type": "material", "effect": "craft", "value": 50, "description": "食尸鬼的爪子", "durability": 0},
            "马鬃": {"type": "material", "effect": "craft", "value": 15, "description": "野马的马鬃", "durability": 0},
            "狼牙": {"type": "material", "effect": "craft", "value": 20, "description": "野狼的獠牙", "durability": 0},
            "僵尸牙齿": {"type": "material", "effect": "craft", "value": 25, "description": "僵尸的牙齿", "durability": 0},
            "水晶龙鳞": {"type": "material", "effect": "craft", "value": 200, "description": "水晶龙的鳞片", "durability": 0},
            "梦境龙鳞": {"type": "material", "effect": "craft", "value": 250, "description": "梦境巨龙的鳞片", "durability": 0},
            "耐热鳞片": {"type": "material", "effect": "craft", "value": 100, "description": "熔岩怪的耐热鳞片", "durability": 0},
            "蜘蛛丝": {"type": "material", "effect": "craft", "value": 15, "description": "洞穴蜘蛛的丝", "durability": 0},
            "强化蜘蛛丝": {"type": "material", "effect": "craft", "value": 40, "description": "矿洞蜘蛛的强化丝", "durability": 0},
            "植物藤蔓": {"type": "material", "effect": "craft", "value": 35, "description": "植物守卫的藤蔓", "durability": 0},
            "熊胆": {"type": "material", "effect": "craft", "value": 50, "description": "巨熊的熊胆，可入药", "durability": 0},
            "巨人心脏": {"type": "material", "effect": "craft", "value": 150, "description": "冰霜巨人的心脏", "durability": 0},
            "石头心脏": {"type": "material", "effect": "craft", "value": 60, "description": "石魔的石制心脏", "durability": 0},
            "毒牙": {"type": "material", "effect": "craft", "value": 25, "description": "洞穴蜘蛛的毒牙", "durability": 0},
            "毒蛙毒液": {"type": "material", "effect": "craft", "value": 35, "description": "毒蛙的毒液", "durability": 0},
            "蛇毒": {"type": "material", "effect": "craft", "value": 40, "description": "沼泽蛇/沙漠巨蛇的毒液", "durability": 0},
            "蝎毒": {"type": "material", "effect": "craft", "value": 30, "description": "沙漠蝎子的毒液", "durability": 0},
            "矿洞毒液": {"type": "material", "effect": "craft", "value": 45, "description": "矿洞蜘蛛的毒液", "durability": 0},
            "书虫毒液": {"type": "material", "effect": "craft", "value": 40, "description": "书虫怪的毒液", "durability": 0},
            "蝎尾针": {"type": "material", "effect": "craft", "value": 35, "description": "沙漠蝎子的尾针，可制作毒药", "durability": 0},
            "傀儡零件": {"type": "material", "effect": "craft", "value": 50, "description": "岩石傀儡的零件", "durability": 0},
            "机器人零件": {"type": "material", "effect": "craft", "value": 60, "description": "机器人/蒸汽机器人的零件", "durability": 0},
            "金属零件": {"type": "material", "effect": "craft", "value": 40, "description": "魔法构装体的金属零件", "durability": 0},
            "赛博格零件": {"type": "material", "effect": "craft", "value": 80, "description": "赛博格的零件", "durability": 0},
            "火炮零件": {"type": "material", "effect": "craft", "value": 70, "description": "火炮手的零件", "durability": 0},
            "高科技零件": {"type": "material", "effect": "craft", "value": 150, "description": "外星生物的高科技零件", "durability": 0},
            "园艺工具": {"type": "material", "effect": "craft", "value": 30, "description": "花园管家的工具", "durability": 0},
            "实验器材": {"type": "material", "effect": "craft", "value": 100, "description": "疯狂科学家的实验器材", "durability": 0},
            "开锁工具": {"type": "material", "effect": "craft", "value": 25, "description": "小偷的开锁工具", "durability": 0},
            "主宰芯片": {"type": "material", "effect": "craft", "value": 200, "description": "AI主宰的智能芯片", "durability": 0},
            "增强芯片": {"type": "material", "effect": "craft", "value": 100, "description": "赛博格的增强芯片", "durability": 0},
            "能量电池": {"type": "material", "effect": "craft", "value": 90, "description": "机器人的能量电池", "durability": 0},
            "精灵粉末": {"type": "material", "effect": "craft", "value": 50, "description": "花精灵的粉末", "durability": 0},
            "外星样本": {"type": "material", "effect": "craft", "value": 120, "description": "外星生物的样本", "durability": 0},
            "木乃伊裹布": {"type": "material", "effect": "craft", "value": 60, "description": "木乃伊的裹布", "durability": 0},
            "骷髅头": {"type": "material", "effect": "craft", "value": 30, "description": "骷髅兵的头骨", "durability": 0},
            "石像鬼之眼": {"type": "material", "effect": "craft", "value": 80, "description": "石像鬼的眼睛", "durability": 0},
            "骨粉": {"type": "material", "effect": "craft", "value": 15, "description": "骷髅兵磨成的骨粉", "durability": 0},
            "坚硬石片": {"type": "material", "effect": "craft", "value": 40, "description": "石像鬼的坚硬石片", "durability": 0},
            "巨石": {"type": "material", "effect": "craft", "value": 30, "description": "古代巨人的巨石", "durability": 0},
            "恶魔之血": {"type": "material", "effect": "craft", "value": 150, "description": "地狱犬的恶魔之血", "durability": 0},
            "辐射血液": {"type": "material", "effect": "craft", "value": 80, "description": "辐射僵尸的血液", "durability": 0},
            "变异组织": {"type": "material", "effect": "craft", "value": 90, "description": "变异生物的组织", "durability": 0},
            "腐肉": {"type": "material", "effect": "craft", "value": 10, "description": "僵尸的腐肉", "durability": 0},
            "火药": {"type": "material", "effect": "craft", "value": 50, "description": "火炮手的火药", "durability": 0}
,        
                        "AI助手": {"type": "treasure", "effect": "sell", "value": 50, "description": "AI助手，珍宝", "durability": 0},
                        "AI芯片": {"type": "treasure", "effect": "sell", "value": 300, "description": "AI芯片，珍宝", "durability": 0},
                        "主大陆凭证": {"type": "treasure", "effect": "sell", "value": 50, "description": "主大陆凭证，珍宝", "durability": 0},
                        "书中世界的纪念品": {"type": "treasure", "effect": "sell", "value": 50, "description": "书中世界的纪念品，珍宝", "durability": 0},
                        "云端消息": {"type": "material", "effect": "craft", "value": 150, "description": "云端消息，材料", "durability": 0},
                        "亡灵的礼物": {"type": "treasure", "effect": "sell", "value": 50, "description": "亡灵的礼物，珍宝", "durability": 0},
                        "亡灵的记忆": {"type": "material", "effect": "craft", "value": 50, "description": "亡灵的记忆，材料", "durability": 0},
                        "住宿": {"type": "treasure", "effect": "sell", "value": 30, "description": "住宿，珍宝", "durability": 0},
                        "光明法术书": {"type": "material", "effect": "craft", "value": 375, "description": "光明法术书，材料", "durability": 0},
                        "光明法杖": {"type": "weapon", "effect": "attack", "value": 525, "description": "光明法杖，武器", "durability": 100, "bonus_attack": 131, "bonus_magic": 175},
                        "公主的祝福": {"type": "treasure", "effect": "sell", "value": 375, "description": "公主的祝福，珍宝", "durability": 0},
                        "冒险勋章": {"type": "accessory", "effect": "passive", "value": 50, "description": "冒险勋章，饰品", "durability": 0, "bonus_attack": 5, "bonus_defense": 5, "bonus_luck": 3},
                        "冒险地图": {"type": "material", "effect": "craft", "value": 150, "description": "冒险地图，材料", "durability": 0},
                        "冠军奖杯": {"type": "accessory", "effect": "passive", "value": 50, "description": "冠军奖杯，饰品", "durability": 0, "bonus_attack": 5, "bonus_defense": 5, "bonus_luck": 3},
                        "冥界信息": {"type": "material", "effect": "craft", "value": 150, "description": "冥界信息，材料", "durability": 0},
                        "冥界通行证": {"type": "treasure", "effect": "sell", "value": 450, "description": "冥界通行证，珍宝", "durability": 0},
                        "冰冻浆果": {"type": "consumable", "effect": "heal", "value": 60, "description": "冰冻浆果，消耗品", "durability": 1},
                        "冰冻魔杖": {"type": "weapon", "effect": "attack", "value": 450, "description": "冰冻魔杖，武器", "durability": 100, "bonus_attack": 112, "bonus_magic": 150},
                        "冰峰之顶的宝石": {"type": "gem", "effect": "gem_bonus", "value": 50, "description": "冰峰之顶的宝石，宝石", "durability": 0, "gem_bonus_attack": 8, "gem_bonus_defense": 8},
                        "冰狼牙": {"type": "material", "effect": "craft", "value": 120, "description": "冰狼牙，材料", "durability": 0},
                        "冰狼皮": {"type": "material", "effect": "craft", "value": 180, "description": "冰狼皮，材料", "durability": 0},
                        "冰系法术书": {"type": "material", "effect": "craft", "value": 375, "description": "冰系法术书，材料", "durability": 0},
                        "冰雕大赛奖杯": {"type": "accessory", "effect": "passive", "value": 50, "description": "冰雕大赛奖杯，饰品", "durability": 0, "bonus_attack": 5, "bonus_defense": 5, "bonus_luck": 3},
                        "冰雕工具": {"type": "material", "effect": "craft", "value": 120, "description": "冰雕工具，材料", "durability": 0},
                        "冰雕艺术品": {"type": "material", "effect": "craft", "value": 150, "description": "冰雕艺术品，材料", "durability": 0},
                        "冰雪结晶": {"type": "gem", "effect": "gem_bonus", "value": 225, "description": "冰雪结晶，宝石", "durability": 0, "gem_bonus_attack": 37, "gem_bonus_defense": 37},
                        "冰雪魔杖": {"type": "weapon", "effect": "attack", "value": 375, "description": "冰雪魔杖，武器", "durability": 100, "bonus_attack": 93, "bonus_magic": 125},
                        "冰霜抗性药水": {"type": "consumable", "effect": "buff_defense", "value": 15, "description": "冰霜抗性药水，消耗品，提升冰霜抗性", "durability": 1},
                        "冰霜法术书": {"type": "material", "effect": "craft", "value": 300, "description": "冰霜法术书，材料", "durability": 0},
                        "冰霜箭": {"type": "material", "effect": "craft", "value": 225, "description": "冰霜箭，材料", "durability": 0},
                        "净化之石": {"type": "gem", "effect": "gem_bonus", "value": 50, "description": "净化之石，宝石", "durability": 0, "gem_bonus_attack": 8, "gem_bonus_defense": 8},
                        "剑道手册": {"type": "weapon", "effect": "attack", "value": 50, "description": "剑道手册，武器", "durability": 100, "bonus_attack": 20},
                        "剑道技巧": {"type": "weapon", "effect": "attack", "value": 225, "description": "剑道技巧，武器", "durability": 100, "bonus_attack": 75},
                        "勇气药水": {"type": "consumable", "effect": "heal", "value": 120, "description": "勇气药水，消耗品", "durability": 1},
                        "北欧神话书籍": {"type": "material", "effect": "craft", "value": 50, "description": "北欧神话书籍，材料", "durability": 0},
                        "占星服务": {"type": "treasure", "effect": "sell", "value": 120, "description": "占星服务，珍宝", "durability": 0},
                        "历史书籍": {"type": "material", "effect": "craft", "value": 180, "description": "历史书籍，材料", "durability": 0},
                        "历史修改器": {"type": "material", "effect": "craft", "value": 50, "description": "历史修改器，材料", "durability": 0},
                        "原始技能手册": {"type": "material", "effect": "craft", "value": 50, "description": "原始技能手册，材料", "durability": 0},
                        "参与奖": {"type": "treasure", "effect": "sell", "value": 50, "description": "参与奖，珍宝", "durability": 0},
                        "友谊徽章": {"type": "accessory", "effect": "passive", "value": 150, "description": "友谊徽章，饰品", "durability": 0, "bonus_attack": 15, "bonus_defense": 15, "bonus_luck": 10},
                        "发光宝石": {"type": "gem", "effect": "gem_bonus", "value": 120, "description": "发光宝石，宝石", "durability": 0, "gem_bonus_attack": 20, "gem_bonus_defense": 20},
                        "发明图纸": {"type": "material", "effect": "craft", "value": 225, "description": "发明图纸，材料", "durability": 0},
                        "古代卷轴": {"type": "material", "effect": "craft", "value": 225, "description": "古代卷轴，材料", "durability": 0},
                        "古代守卫徽章": {"type": "accessory", "effect": "passive", "value": 150, "description": "古代守卫徽章，饰品", "durability": 0, "bonus_attack": 15, "bonus_defense": 15, "bonus_luck": 10},
                        "古代知识卷轴": {"type": "material", "effect": "craft", "value": 50, "description": "古代知识卷轴，材料", "durability": 0},
                        "史前 artifact": {"type": "treasure", "effect": "sell", "value": 50, "description": "史前 artifact，珍宝", "durability": 0},
                        "向导服务": {"type": "treasure", "effect": "sell", "value": 225, "description": "向导服务，珍宝", "durability": 0},
                        "命运之线": {"type": "gem", "effect": "gem_bonus", "value": 300, "description": "命运之线，宝石", "durability": 0, "gem_bonus_luck": 37},
                        "命运水晶": {"type": "gem", "effect": "gem_bonus", "value": 180, "description": "命运水晶，宝石", "durability": 0, "gem_bonus_luck": 22},
                        "商人徽章": {"type": "accessory", "effect": "passive", "value": 300, "description": "商人徽章，饰品", "durability": 0, "bonus_attack": 30, "bonus_defense": 30, "bonus_luck": 20},
                        "商人的感谢": {"type": "treasure", "effect": "sell", "value": 150, "description": "商人的感谢，珍宝", "durability": 0},
                        "噩梦征服者徽章": {"type": "accessory", "effect": "passive", "value": 50, "description": "噩梦征服者徽章，饰品", "durability": 0, "bonus_attack": 5, "bonus_defense": 5, "bonus_luck": 3},
                        "回忆水晶": {"type": "gem", "effect": "gem_bonus", "value": 225, "description": "回忆水晶，宝石", "durability": 0, "gem_bonus_magic": 45},
                        "回忆结晶": {"type": "gem", "effect": "gem_bonus", "value": 180, "description": "回忆结晶，宝石", "durability": 0, "gem_bonus_magic": 36},
                        "园艺大师证书": {"type": "treasure", "effect": "sell", "value": 50, "description": "园艺大师证书，珍宝", "durability": 0},
                        "图腾": {"type": "material", "effect": "craft", "value": 180, "description": "图腾，材料", "durability": 0},
                        "圣水": {"type": "consumable", "effect": "heal", "value": 225, "description": "圣水，消耗品", "durability": 1},
                        "地下凭证": {"type": "treasure", "effect": "sell", "value": 50, "description": "地下凭证，珍宝", "durability": 0},
                        "地狱之火": {"type": "material", "effect": "craft", "value": 50, "description": "地狱之火，材料", "durability": 0},
                        "城市通行证": {"type": "treasure", "effect": "sell", "value": 75, "description": "城市通行证，珍宝", "durability": 0},
                        "外星 artifact": {"type": "treasure", "effect": "sell", "value": 450, "description": "外星 artifact，珍宝", "durability": 0},
                        "外星纪念品": {"type": "treasure", "effect": "sell", "value": 50, "description": "外星纪念品，珍宝", "durability": 0},
                        "天使之羽": {"type": "material", "effect": "craft", "value": 225, "description": "天使之羽，材料", "durability": 0},
                        "天文仪器": {"type": "material", "effect": "craft", "value": 225, "description": "天文仪器，材料", "durability": 0},
                        "天气控制器": {"type": "material", "effect": "craft", "value": 50, "description": "天气控制器，材料", "durability": 0},
                        "天空凭证": {"type": "treasure", "effect": "sell", "value": 50, "description": "天空凭证，珍宝", "durability": 0},
                        "天空剑": {"type": "weapon", "effect": "attack", "value": 300, "description": "天空剑，武器", "durability": 100, "bonus_attack": 100},
                        "天空水晶": {"type": "gem", "effect": "gem_bonus", "value": 300, "description": "天空水晶，宝石", "durability": 0, "gem_bonus_attack": 50, "gem_bonus_defense": 50},
                        "奇幻生物的礼物": {"type": "treasure", "effect": "sell", "value": 50, "description": "奇幻生物的礼物，珍宝", "durability": 0},
                        "奇幻种子": {"type": "consumable", "effect": "heal", "value": 75, "description": "奇幻种子，消耗品", "durability": 1},
                        "女王的祝福": {"type": "treasure", "effect": "sell", "value": 450, "description": "女王的祝福，珍宝", "durability": 0},
                        "学院机密卷轴": {"type": "material", "effect": "craft", "value": 50, "description": "学院机密卷轴，材料", "durability": 0},
                        "宇宙祝福": {"type": "treasure", "effect": "sell", "value": 375, "description": "宇宙祝福，珍宝", "durability": 0},
                        "守墓人的钥匙": {"type": "material", "effect": "craft", "value": 300, "description": "守墓人的钥匙，材料", "durability": 0},
                        "守护符": {"type": "accessory", "effect": "passive", "value": 150, "description": "守护符，饰品", "durability": 0, "bonus_defense": 30},
                        "宗教圣物": {"type": "gem", "effect": "gem_bonus", "value": 300, "description": "宗教圣物，宝石", "durability": 0, "gem_bonus_attack": 50, "gem_bonus_defense": 50},
                        "宝石戒指": {"type": "gem", "effect": "gem_bonus", "value": 225, "description": "宝石戒指，宝石", "durability": 0, "gem_bonus_attack": 37, "gem_bonus_defense": 37},
                        "宝石法杖": {"type": "weapon", "effect": "attack", "value": 450, "description": "宝石法杖，武器", "durability": 100, "bonus_attack": 112, "bonus_magic": 150},
                        "宝石项链": {"type": "gem", "effect": "gem_bonus", "value": 300, "description": "宝石项链，宝石", "durability": 0, "gem_bonus_attack": 50, "gem_bonus_defense": 50},
                        "实验材料": {"type": "material", "effect": "craft", "value": 150, "description": "实验材料，材料", "durability": 0},
                        "审判之剑": {"type": "weapon", "effect": "attack", "value": 50, "description": "审判之剑，武器", "durability": 100, "bonus_attack": 20},
                        "宫廷勋章": {"type": "accessory", "effect": "passive", "value": 50, "description": "宫廷勋章，饰品", "durability": 0, "bonus_attack": 5, "bonus_defense": 5, "bonus_luck": 3},
                        "宫廷法术书": {"type": "material", "effect": "craft", "value": 375, "description": "宫廷法术书，材料", "durability": 0},
                        "寻宝工具": {"type": "material", "effect": "craft", "value": 150, "description": "寻宝工具，材料", "durability": 0},
                        "岛上特产": {"type": "consumable", "effect": "heal", "value": 30, "description": "岛上特产，消耗品", "durability": 1},
                        "工业产品": {"type": "material", "effect": "craft", "value": 180, "description": "工业产品，材料", "durability": 0},
                        "工人工具": {"type": "material", "effect": "craft", "value": 75, "description": "工人工具，材料", "durability": 0},
                        "工人领袖徽章": {"type": "accessory", "effect": "passive", "value": 50, "description": "工人领袖徽章，饰品", "durability": 0, "bonus_attack": 5, "bonus_defense": 5, "bonus_luck": 3},
                        "工会徽章": {"type": "accessory", "effect": "passive", "value": 150, "description": "工会徽章，饰品", "durability": 0, "bonus_attack": 15, "bonus_defense": 15, "bonus_luck": 10},
                        "工厂参观纪念章": {"type": "accessory", "effect": "passive", "value": 50, "description": "工厂参观纪念章，饰品", "durability": 0, "bonus_attack": 5, "bonus_defense": 5, "bonus_luck": 3},
                        "幸福结晶": {"type": "gem", "effect": "gem_bonus", "value": 225, "description": "幸福结晶，宝石", "durability": 0, "gem_bonus_luck": 28},
                        "幸运符": {"type": "material", "effect": "craft", "value": 225, "description": "幸运符，材料", "durability": 0},
                        "幽灵护符": {"type": "accessory", "effect": "passive", "value": 375, "description": "幽灵护符，饰品", "durability": 0, "bonus_attack": 37, "bonus_defense": 37, "bonus_luck": 25},
                        "幽灵礼服": {"type": "armor", "effect": "defense", "value": 50, "description": "幽灵礼服，防具", "durability": 120, "bonus_defense": 16},
                        "建筑图纸": {"type": "material", "effect": "craft", "value": 225, "description": "建筑图纸，材料", "durability": 0},
                        "强力毒药": {"type": "material", "effect": "craft", "value": 225, "description": "强力毒药，材料", "durability": 0},
                        "强化机械装置": {"type": "material", "effect": "craft", "value": 50, "description": "强化机械装置，材料", "durability": 0},
                        "强化机械零件": {"type": "material", "effect": "craft", "value": 50, "description": "强化机械零件，材料", "durability": 0},
                        "往生符": {"type": "material", "effect": "craft", "value": 120, "description": "往生符，材料", "durability": 0},
                        "忍术卷轴": {"type": "material", "effect": "craft", "value": 375, "description": "忍术卷轴，材料", "durability": 0},
                        "忍者套装": {"type": "armor", "effect": "defense", "value": 50, "description": "忍者套装，防具", "durability": 120, "bonus_defense": 16},
                        "快乐精华": {"type": "gem", "effect": "gem_bonus", "value": 120, "description": "快乐精华，宝石", "durability": 0, "gem_bonus_luck": 15},
                        "快乐精灵的礼物": {"type": "treasure", "effect": "sell", "value": 50, "description": "快乐精灵的礼物，珍宝", "durability": 0},
                        "快乐药水": {"type": "consumable", "effect": "heal", "value": 150, "description": "快乐药水，消耗品", "durability": 1},
                        "情报": {"type": "material", "effect": "craft", "value": 45, "description": "情报，材料", "durability": 0},
                        "抗毒药水": {"type": "consumable", "effect": "special", "value": 0, "description": "抗毒药水，消耗品，解除中毒状态", "durability": 1},
                        "拍卖入场券": {"type": "material", "effect": "craft", "value": 150, "description": "拍卖入场券，材料", "durability": 0},
                        "掠夺品": {"type": "treasure", "effect": "sell", "value": 150, "description": "掠夺品，珍宝", "durability": 0},
                        "日本刀": {"type": "weapon", "effect": "attack", "value": 300, "description": "日本刀，武器", "durability": 100, "bonus_attack": 100},
                        "时空装置": {"type": "material", "effect": "craft", "value": 450, "description": "时空装置，材料", "durability": 0},
                        "时空转换器": {"type": "material", "effect": "craft", "value": 600, "description": "时空转换器，材料", "durability": 0},
                        "时间修复器": {"type": "material", "effect": "craft", "value": 50, "description": "时间修复器，材料", "durability": 0},
                        "时间凭证": {"type": "treasure", "effect": "sell", "value": 50, "description": "时间凭证，珍宝", "durability": 0},
                        "时间地图": {"type": "material", "effect": "craft", "value": 225, "description": "时间地图，材料", "durability": 0},
                        "时间祝福": {"type": "treasure", "effect": "sell", "value": 300, "description": "时间祝福，珍宝", "durability": 0},
                        "时间钥匙": {"type": "material", "effect": "craft", "value": 450, "description": "时间钥匙，材料", "durability": 0},
                        "星图": {"type": "material", "effect": "craft", "value": 150, "description": "星图，材料", "durability": 0},
                        "星象图": {"type": "material", "effect": "craft", "value": 50, "description": "星象图，材料", "durability": 0},
                        "星际商品": {"type": "material", "effect": "craft", "value": 375, "description": "星际商品，材料", "durability": 0},
                        "星际地图": {"type": "material", "effect": "craft", "value": 300, "description": "星际地图，材料", "durability": 0},
                        "星际生物的礼物": {"type": "treasure", "effect": "sell", "value": 50, "description": "星际生物的礼物，珍宝", "durability": 0},
                        "星际飞船模型": {"type": "accessory", "effect": "passive", "value": 50, "description": "星际飞船模型，饰品", "durability": 0, "bonus_attack": 5, "bonus_defense": 5, "bonus_luck": 3},
                        "星际食品": {"type": "consumable", "effect": "heal", "value": 75, "description": "星际食品，消耗品", "durability": 1},
                        "暗影法术书": {"type": "material", "effect": "craft", "value": 375, "description": "暗影法术书，材料", "durability": 0},
                        "最终试炼证明": {"type": "treasure", "effect": "sell", "value": 50, "description": "最终试炼证明，珍宝", "durability": 0},
                        "朗姆酒": {"type": "consumable", "effect": "heal", "value": 45, "description": "朗姆酒，消耗品", "durability": 1},
                        "未来水晶球": {"type": "gem", "effect": "gem_bonus", "value": 50, "description": "未来水晶球，宝石", "durability": 0, "gem_bonus_magic": 10},
                        "未来科技装置": {"type": "material", "effect": "craft", "value": 50, "description": "未来科技装置，材料", "durability": 0},
                        "本地地图": {"type": "material", "effect": "craft", "value": 75, "description": "本地地图，材料", "durability": 0},
                        "机关图纸": {"type": "material", "effect": "craft", "value": 50, "description": "机关图纸，材料", "durability": 0},
                        "机械助手": {"type": "treasure", "effect": "sell", "value": 600, "description": "机械助手，珍宝", "durability": 0},
                        "机械宠物": {"type": "treasure", "effect": "sell", "value": 450, "description": "机械宠物，珍宝", "durability": 0},
                        "村庄特产": {"type": "consumable", "effect": "heal", "value": 20, "description": "村庄特产，消耗品", "durability": 1},
                        "梦境凭证": {"type": "treasure", "effect": "sell", "value": 50, "description": "梦境凭证，珍宝", "durability": 0},
                        "梦境控制器": {"type": "material", "effect": "craft", "value": 600, "description": "梦境控制器，材料", "durability": 0},
                        "梦境解读": {"type": "treasure", "effect": "sell", "value": 225, "description": "梦境解读，珍宝", "durability": 0},
                        "梦境魔法书": {"type": "material", "effect": "craft", "value": 300, "description": "梦境魔法书，材料", "durability": 0},
                        "梦想实现石": {"type": "material", "effect": "craft", "value": 50, "description": "梦想实现石，材料", "durability": 0},
                        "梦想精华": {"type": "gem", "effect": "gem_bonus", "value": 225, "description": "梦想精华，宝石", "durability": 0, "gem_bonus_attack": 37, "gem_bonus_defense": 37},
                        "森林护符": {"type": "accessory", "effect": "passive", "value": 300, "description": "森林护符，饰品", "durability": 0, "bonus_attack": 30, "bonus_defense": 30, "bonus_luck": 20},
                        "植物样本": {"type": "material", "effect": "craft", "value": 75, "description": "植物样本，材料", "durability": 0},
                        "樱花徽章": {"type": "accessory", "effect": "passive", "value": 50, "description": "樱花徽章，饰品", "durability": 0, "bonus_attack": 5, "bonus_defense": 5, "bonus_luck": 3},
                        "正义之剑": {"type": "weapon", "effect": "attack", "value": 375, "description": "正义之剑，武器", "durability": 100, "bonus_attack": 125},
                        "正义徽章": {"type": "accessory", "effect": "passive", "value": 225, "description": "正义徽章，饰品", "durability": 0, "bonus_attack": 45},
                        "武器": {"type": "material", "effect": "craft", "value": 225, "description": "武器，材料", "durability": 0},
                        "毒腺": {"type": "material", "effect": "craft", "value": 120, "description": "毒腺，材料", "durability": 0},
                        "毒草": {"type": "material", "effect": "craft", "value": 50, "description": "毒草，材料", "durability": 0},
                        "水下呼吸药水": {"type": "consumable", "effect": "special", "value": 0, "description": "水下呼吸药水，消耗品，允许水下呼吸", "durability": 1},
                        "水下建筑材料": {"type": "material", "effect": "craft", "value": 225, "description": "水下建筑材料，材料", "durability": 0},
                        "水下灯": {"type": "material", "effect": "craft", "value": 150, "description": "水下灯，材料", "durability": 0},
                        "水晶工具": {"type": "gem", "effect": "gem_bonus", "value": 225, "description": "水晶工具，宝石", "durability": 0, "gem_bonus_attack": 37, "gem_bonus_defense": 37},
                        "治疗水晶": {"type": "gem", "effect": "gem_bonus", "value": 270, "description": "治疗水晶，宝石", "durability": 0, "gem_bonus_hp": 90},
                        "沼泽护符": {"type": "accessory", "effect": "passive", "value": 270, "description": "沼泽护符，饰品", "durability": 0, "bonus_attack": 27, "bonus_defense": 27, "bonus_luck": 18},
                        "浪漫纪念品": {"type": "treasure", "effect": "sell", "value": 50, "description": "浪漫纪念品，珍宝", "durability": 0},
                        "浪漫花瓣": {"type": "material", "effect": "craft", "value": 120, "description": "浪漫花瓣，材料", "durability": 0},
                        "海图": {"type": "material", "effect": "craft", "value": 150, "description": "海图，材料", "durability": 0},
                        "海洋之心": {"type": "material", "effect": "craft", "value": 50, "description": "海洋之心，材料", "durability": 0},
                        "海洋护符": {"type": "accessory", "effect": "passive", "value": 450, "description": "海洋护符，饰品", "durability": 0, "bonus_attack": 45, "bonus_defense": 45, "bonus_luck": 30},
                        "海洋水晶": {"type": "gem", "effect": "gem_bonus", "value": 300, "description": "海洋水晶，宝石", "durability": 0, "gem_bonus_attack": 50, "gem_bonus_defense": 50},
                        "海洋法术书": {"type": "material", "effect": "craft", "value": 450, "description": "海洋法术书，材料", "durability": 0},
                        "海洋生物样本": {"type": "material", "effect": "craft", "value": 150, "description": "海洋生物样本，材料", "durability": 0},
                        "海盗战利品": {"type": "treasure", "effect": "sell", "value": 120, "description": "海盗战利品，珍宝", "durability": 0},
                        "海盗料理": {"type": "consumable", "effect": "heal", "value": 30, "description": "海盗料理，消耗品", "durability": 1},
                        "海盗船长帽": {"type": "material", "effect": "craft", "value": 50, "description": "海盗船长帽，材料", "durability": 0},
                        "海草": {"type": "consumable", "effect": "heal", "value": 20, "description": "海草，消耗品", "durability": 1},
                        "海鲜": {"type": "consumable", "effect": "heal", "value": 22, "description": "海鲜，消耗品", "durability": 1},
                        "游牧饰品": {"type": "material", "effect": "craft", "value": 60, "description": "游牧饰品，材料", "durability": 0},
                        "潮汐法杖": {"type": "weapon", "effect": "attack", "value": 525, "description": "潮汐法杖，武器", "durability": 100, "bonus_attack": 131, "bonus_magic": 175},
                        "灵魂之光": {"type": "gem", "effect": "gem_bonus", "value": 180, "description": "灵魂之光，宝石", "durability": 0, "gem_bonus_hp": 60},
                        "灵魂指引符": {"type": "material", "effect": "craft", "value": 225, "description": "灵魂指引符，材料", "durability": 0},
                        "灵魂精华": {"type": "gem", "effect": "gem_bonus", "value": 180, "description": "灵魂精华，宝石", "durability": 0, "gem_bonus_hp": 60},
                        "熔岩样本": {"type": "material", "effect": "craft", "value": 225, "description": "熔岩样本，材料", "durability": 0},
                        "爱情信物": {"type": "treasure", "effect": "sell", "value": 180, "description": "爱情信物，珍宝", "durability": 0},
                        "爱情药水": {"type": "consumable", "effect": "heal", "value": 225, "description": "爱情药水，消耗品", "durability": 1},
                        "爵位": {"type": "treasure", "effect": "sell", "value": 750, "description": "爵位，珍宝", "durability": 0},
                        "狩猎技巧": {"type": "material", "effect": "craft", "value": 150, "description": "狩猎技巧，材料", "durability": 0},
                        "现实平衡器": {"type": "material", "effect": "craft", "value": 50, "description": "现实平衡器，材料", "durability": 0},
                        "珍珠": {"type": "material", "effect": "craft", "value": 90, "description": "珍珠，材料", "durability": 0},
                        "珍珠项链": {"type": "accessory", "effect": "passive", "value": 375, "description": "珍珠项链，饰品", "durability": 0, "bonus_attack": 37, "bonus_defense": 37, "bonus_luck": 25},
                        "生存工具": {"type": "material", "effect": "craft", "value": 120, "description": "生存工具，材料", "durability": 0},
                        "皇家魔法杖": {"type": "weapon", "effect": "attack", "value": 450, "description": "皇家魔法杖，武器", "durability": 100, "bonus_attack": 112, "bonus_magic": 150},
                        "监狱钥匙": {"type": "material", "effect": "craft", "value": 150, "description": "监狱钥匙，材料", "durability": 0},
                        "盔甲": {"type": "armor", "effect": "defense", "value": 225, "description": "盔甲，防具", "durability": 120, "bonus_defense": 75},
                        "矮人锻造手册": {"type": "material", "effect": "craft", "value": 50, "description": "矮人锻造手册，材料", "durability": 0},
                        "矿工装备": {"type": "material", "effect": "craft", "value": 225, "description": "矿工装备，材料", "durability": 0},
                        "矿洞地图": {"type": "material", "effect": "craft", "value": 150, "description": "矿洞地图，材料", "durability": 0},
                        "研究材料": {"type": "material", "effect": "craft", "value": 90, "description": "研究材料，材料", "durability": 0},
                        "研究笔记": {"type": "material", "effect": "craft", "value": 225, "description": "研究笔记，材料", "durability": 0},
                        "研究资料": {"type": "material", "effect": "craft", "value": 150, "description": "研究资料，材料", "durability": 0},
                        "神圣祝福": {"type": "treasure", "effect": "sell", "value": 375, "description": "神圣祝福，珍宝", "durability": 0},
                        "神格": {"type": "treasure", "effect": "sell", "value": 50, "description": "神格，珍宝", "durability": 0},
                        "神的祝福": {"type": "treasure", "effect": "sell", "value": 50, "description": "神的祝福，珍宝", "durability": 0},
                        "神秘 artifact": {"type": "treasure", "effect": "sell", "value": 50, "description": "神秘 artifact，珍宝", "durability": 0},
                        "神秘卷轴": {"type": "material", "effect": "craft", "value": 300, "description": "神秘卷轴，材料", "durability": 0},
                        "神秘宝物": {"type": "treasure", "effect": "sell", "value": 50, "description": "神秘宝物，珍宝", "durability": 0},
                        "神秘生物的礼物": {"type": "treasure", "effect": "sell", "value": 50, "description": "神秘生物的礼物，珍宝", "durability": 0},
                        "神秘符咒": {"type": "material", "effect": "craft", "value": 225, "description": "神秘符咒，材料", "durability": 0},
                        "神秘结晶": {"type": "gem", "effect": "gem_bonus", "value": 300, "description": "神秘结晶，宝石", "durability": 0, "gem_bonus_attack": 50, "gem_bonus_defense": 50},
                        "祭司的祝福": {"type": "treasure", "effect": "sell", "value": 300, "description": "祭司的祝福，珍宝", "durability": 0},
                        "禁书": {"type": "material", "effect": "craft", "value": 50, "description": "禁书，材料", "durability": 0},
                        "禁术卷轴": {"type": "material", "effect": "craft", "value": 50, "description": "禁术卷轴，材料", "durability": 0},
                        "种子": {"type": "consumable", "effect": "heal", "value": 22, "description": "种子，消耗品", "durability": 1},
                        "科学仪器": {"type": "material", "effect": "craft", "value": 300, "description": "科学仪器，材料", "durability": 0},
                        "科学奖杯": {"type": "accessory", "effect": "passive", "value": 50, "description": "科学奖杯，饰品", "durability": 0, "bonus_attack": 5, "bonus_defense": 5, "bonus_luck": 3},
                        "科技产品": {"type": "material", "effect": "craft", "value": 225, "description": "科技产品，材料", "durability": 0},
                        "科技展览纪念品": {"type": "treasure", "effect": "sell", "value": 50, "description": "科技展览纪念品，珍宝", "durability": 0},
                        "秘密信息": {"type": "material", "effect": "craft", "value": 75, "description": "秘密信息，材料", "durability": 0},
                        "稀有商品": {"type": "material", "effect": "craft", "value": 375, "description": "稀有商品，材料", "durability": 0},
                        "稀有商品目录": {"type": "material", "effect": "craft", "value": 225, "description": "稀有商品目录，材料", "durability": 0},
                        "稀有宝石": {"type": "gem", "effect": "gem_bonus", "value": 375, "description": "稀有宝石，宝石", "durability": 0, "gem_bonus_attack": 62, "gem_bonus_defense": 62},
                        "稀有货物": {"type": "material", "effect": "craft", "value": 225, "description": "稀有货物，材料", "durability": 0},
                        "稀有鱼": {"type": "consumable", "effect": "heal", "value": 60, "description": "稀有鱼，消耗品", "durability": 1},
                        "稳定符": {"type": "material", "effect": "craft", "value": 300, "description": "稳定符，材料", "durability": 0},
                        "空间碎片": {"type": "material", "effect": "craft", "value": 225, "description": "空间碎片，材料", "durability": 0},
                        "童心药水": {"type": "consumable", "effect": "heal", "value": 120, "description": "童心药水，消耗品", "durability": 1},
                        "精灵之舞的祝福": {"type": "treasure", "effect": "sell", "value": 50, "description": "精灵之舞的祝福，珍宝", "durability": 0},
                        "精灵护甲": {"type": "armor", "effect": "defense", "value": 525, "description": "精灵护甲，防具", "durability": 120, "bonus_defense": 175},
                        "精灵武器": {"type": "material", "effect": "craft", "value": 600, "description": "精灵武器，材料", "durability": 0},
                        "精灵王冠": {"type": "accessory", "effect": "passive", "value": 750, "description": "精灵王冠，饰品", "durability": 0, "bonus_attack": 75, "bonus_defense": 75, "bonus_luck": 50},
                        "精灵玩具": {"type": "accessory", "effect": "passive", "value": 75, "description": "精灵玩具，饰品", "durability": 0, "bonus_attack": 7, "bonus_defense": 7, "bonus_luck": 5},
                        "精灵礼服": {"type": "armor", "effect": "defense", "value": 50, "description": "精灵礼服，防具", "durability": 120, "bonus_defense": 16},
                        "精灵饰品": {"type": "material", "effect": "craft", "value": 300, "description": "精灵饰品，材料", "durability": 0},
                        "精灵魔法书": {"type": "material", "effect": "craft", "value": 50, "description": "精灵魔法书，材料", "durability": 0},
                        "考古发现": {"type": "treasure", "effect": "sell", "value": 150, "description": "考古发现，珍宝", "durability": 0},
                        "能量水晶": {"type": "gem", "effect": "gem_bonus", "value": 180, "description": "能量水晶，宝石", "durability": 0, "gem_bonus_attack": 36},
                        "自然之语": {"type": "material", "effect": "craft", "value": 50, "description": "自然之语，材料", "durability": 0},
                        "航海图": {"type": "material", "effect": "craft", "value": 180, "description": "航海图，材料", "durability": 0},
                        "航海装备": {"type": "material", "effect": "craft", "value": 225, "description": "航海装备，材料", "durability": 0},
                        "花卉": {"type": "consumable", "effect": "heal", "value": 20, "description": "花卉，消耗品", "durability": 1},
                        "花粉": {"type": "consumable", "effect": "heal", "value": 25, "description": "花粉，消耗品", "durability": 1},
                        "花精灵的祝福": {"type": "treasure", "effect": "sell", "value": 50, "description": "花精灵的祝福，珍宝", "durability": 0},
                        "英雄任务完成证明": {"type": "treasure", "effect": "sell", "value": 50, "description": "英雄任务完成证明，珍宝", "durability": 0},
                        "英雄套装": {"type": "armor", "effect": "defense", "value": 50, "description": "英雄套装，防具", "durability": 120, "bonus_defense": 16},
                        "英雄训练": {"type": "treasure", "effect": "sell", "value": 300, "description": "英雄训练，珍宝", "durability": 0},
                        "萨满药水": {"type": "consumable", "effect": "heal", "value": 120, "description": "萨满药水，消耗品", "durability": 1},
                        "萨满道具": {"type": "material", "effect": "craft", "value": 150, "description": "萨满道具，材料", "durability": 0},
                        "解毒药水": {"type": "consumable", "effect": "special", "value": 0, "description": "解毒药水，消耗品，解除中毒状态", "durability": 1},
                        "记忆之书": {"type": "material", "effect": "craft", "value": 150, "description": "记忆之书，材料", "durability": 0},
                        "记忆药水": {"type": "consumable", "effect": "heal", "value": 150, "description": "记忆药水，消耗品", "durability": 1},
                        "诅咒碎片": {"type": "material", "effect": "craft", "value": 120, "description": "诅咒碎片，材料", "durability": 0},
                        "象形文字词典": {"type": "material", "effect": "craft", "value": 50, "description": "象形文字词典，材料", "durability": 0},
                        "贤者的祝福": {"type": "treasure", "effect": "sell", "value": 375, "description": "贤者的祝福，珍宝", "durability": 0},
                        "贵族徽章": {"type": "accessory", "effect": "passive", "value": 300, "description": "贵族徽章，饰品", "durability": 0, "bonus_attack": 30, "bonus_defense": 30, "bonus_luck": 20},
                        "迷宫地图": {"type": "material", "effect": "craft", "value": 225, "description": "迷宫地图，材料", "durability": 0},
                        "遗迹地图": {"type": "material", "effect": "craft", "value": 225, "description": "遗迹地图，材料", "durability": 0},
                        "部落勇士徽章": {"type": "accessory", "effect": "passive", "value": 50, "description": "部落勇士徽章，饰品", "durability": 0, "bonus_attack": 5, "bonus_defense": 5, "bonus_luck": 3},
                        "部落徽章": {"type": "accessory", "effect": "passive", "value": 225, "description": "部落徽章，饰品", "durability": 0, "bonus_attack": 22, "bonus_defense": 22, "bonus_luck": 15},
                        "部落特产": {"type": "consumable", "effect": "heal", "value": 45, "description": "部落特产，消耗品", "durability": 1},
                        "重建许可证": {"type": "treasure", "effect": "sell", "value": 50, "description": "重建许可证，珍宝", "durability": 0},
                        "野兽皮毛": {"type": "material", "effect": "craft", "value": 75, "description": "野兽皮毛，材料", "durability": 0},
                        "金字塔模型": {"type": "accessory", "effect": "passive", "value": 300, "description": "金字塔模型，饰品", "durability": 0, "bonus_attack": 30, "bonus_defense": 30, "bonus_luck": 20},
                        "铁矿": {"type": "material", "effect": "craft", "value": 22, "description": "铁矿，材料", "durability": 0},
                        "锻造大赛奖牌": {"type": "accessory", "effect": "passive", "value": 50, "description": "锻造大赛奖牌，饰品", "durability": 0, "bonus_attack": 5, "bonus_defense": 5, "bonus_luck": 3},
                        "镇魂石": {"type": "gem", "effect": "gem_bonus", "value": 270, "description": "镇魂石，宝石", "durability": 0, "gem_bonus_attack": 45, "gem_bonus_defense": 45},
                        "防水装备": {"type": "material", "effect": "craft", "value": 300, "description": "防水装备，材料", "durability": 0},
                        "集市通行证": {"type": "treasure", "effect": "sell", "value": 75, "description": "集市通行证，珍宝", "durability": 0},
                        "雪人友好护符": {"type": "accessory", "effect": "passive", "value": 300, "description": "雪人友好护符，饰品", "durability": 0, "bonus_attack": 30, "bonus_defense": 30, "bonus_luck": 20},
                        "雪精灵的祝福": {"type": "treasure", "effect": "sell", "value": 50, "description": "雪精灵的祝福，珍宝", "durability": 0},
                        "预言卷轴": {"type": "material", "effect": "craft", "value": 300, "description": "预言卷轴，材料", "durability": 0},
                        "飞行云朵": {"type": "treasure", "effect": "sell", "value": 50, "description": "飞行云朵，珍宝", "durability": 0},
                        "飞行坐骑": {"type": "treasure", "effect": "sell", "value": 750, "description": "飞行坐骑，珍宝", "durability": 0},
                        "飞行技巧指南": {"type": "material", "effect": "craft", "value": 50, "description": "飞行技巧指南，材料", "durability": 0},
                        "飞行比赛奖杯": {"type": "accessory", "effect": "passive", "value": 50, "description": "飞行比赛奖杯，饰品", "durability": 0, "bonus_speed": 6},
                        "飞行法术书": {"type": "material", "effect": "craft", "value": 450, "description": "飞行法术书，材料", "durability": 0},
                        "飞行盔甲": {"type": "armor", "effect": "defense", "value": 375, "description": "飞行盔甲，防具", "durability": 120, "bonus_defense": 125},
                        "飞行祝福": {"type": "treasure", "effect": "sell", "value": 300, "description": "飞行祝福，珍宝", "durability": 0},
                        "飞行符": {"type": "material", "effect": "craft", "value": 120, "description": "飞行符，材料", "durability": 0},
                        "飞行许可证": {"type": "treasure", "effect": "sell", "value": 225, "description": "飞行许可证，珍宝", "durability": 0},
                        "飞行课程": {"type": "treasure", "effect": "sell", "value": 150, "description": "飞行课程，珍宝", "durability": 0},
                        "食物": {"type": "consumable", "effect": "heal", "value": 20, "description": "食物，消耗品", "durability": 1},
                        "驯龙棒": {"type": "treasure", "effect": "sell", "value": 300, "description": "驯龙棒，珍宝", "durability": 0},
                        "驱魔剑": {"type": "weapon", "effect": "attack", "value": 450, "description": "驱魔剑，武器", "durability": 100, "bonus_attack": 150},
                        "驱魔符": {"type": "material", "effect": "craft", "value": 300, "description": "驱魔符，材料", "durability": 0},
                        "驱魔药水": {"type": "consumable", "effect": "heal", "value": 80, "description": "驱魔药水，消耗品，对亡灵系敌人有奇效", "durability": 1},
                        "骑士装备": {"type": "material", "effect": "craft", "value": 300, "description": "骑士装备，材料", "durability": 0},
                        "骑士训练": {"type": "treasure", "effect": "sell", "value": 225, "description": "骑士训练，珍宝", "durability": 0},
                        "骑士铠甲": {"type": "armor", "effect": "defense", "value": 50, "description": "骑士铠甲，防具", "durability": 120, "bonus_defense": 16},
                        "魔法果实": {"type": "consumable", "effect": "mana", "value": 120, "description": "魔法果实，消耗品", "durability": 1},
                        "魔法植物": {"type": "material", "effect": "craft", "value": 50, "description": "魔法植物，材料", "durability": 0},
                        "鱼": {"type": "consumable", "effect": "heal", "value": 20, "description": "鱼，消耗品", "durability": 1},
                        "黑暗祭坛": {"type": "treasure", "effect": "sell", "value": 50, "description": "黑暗祭坛，珍宝", "durability": 0},
                        "龙伙伴": {"type": "treasure", "effect": "sell", "value": 50, "description": "龙伙伴，珍宝", "durability": 0},
                        "龙族研究笔记": {"type": "material", "effect": "craft", "value": 450, "description": "龙族研究笔记，材料", "durability": 0},
                        "龙蛋": {"type": "treasure", "effect": "sell", "value": 50, "description": "龙蛋，珍宝", "durability": 0},
                        "龙语词典": {"type": "material", "effect": "craft", "value": 375, "description": "龙语词典，材料", "durability": 0},
                        "龙鞍": {"type": "treasure", "effect": "sell", "value": 750, "description": "龙鞍，珍宝", "durability": 0},
                        "龙食": {"type": "consumable", "effect": "heal", "value": 112, "description": "龙食，消耗品", "durability": 1},
                        "龙骑士套装": {"type": "armor", "effect": "defense", "value": 50, "description": "龙骑士套装，防具", "durability": 120, "bonus_defense": 16},
                        "龙骑士徽章": {"type": "accessory", "effect": "passive", "value": 50, "description": "龙骑士徽章，饰品", "durability": 0, "bonus_attack": 5, "bonus_defense": 5, "bonus_luck": 3},
                        "龙骑士装备": {"type": "material", "effect": "craft", "value": 450, "description": "龙骑士装备，材料", "durability": 0},
                        "龙鳞护甲": {"type": "armor", "effect": "defense", "value": 600, "description": "龙鳞护甲，防具", "durability": 120, "bonus_defense": 200},
}
        ...
        self.crafting_recipes = {
            ...
            "暗影密钥": {
                "type": "key",
                "ingredients": {"主大陆凭证": 1, "地下凭证": 1, "天空凭证": 1, "时间凭证": 1, "梦境凭证": 1},
                "result": {"item": "暗影密钥", "quantity": 1},
                "description": "用于解锁暗影之地的密钥"
            },
        }
        ...
        self.gem_recipes = {
            ...
            "魔法黄宝石": {
                "type": "gem",
                "ingredients": {"普通黄宝石": 2, "魔法宝石碎片": 3, "金锭": 2},
            ...
            "稀有黄宝石": {
                "type": "gem",
                "ingredients": {"魔法黄宝石": 2, "稀有宝石碎片": 3, "金锭": 3, "龙鳞": 1},
            ...
            "史诗黄宝石": {
                "type": "gem",
                "ingredients": {"稀有黄宝石": 2, "史诗宝石碎片": 3, "金锭": 4, "龙之宝石": 1},
            ...
            "传说黄宝石": {
                "type": "gem",
                "ingredients": {"史诗黄宝石": 2, "传说宝石碎片": 3, "金锭": 5, "创世神剑": 1},
            ...
            "神话黄宝石": {
                "type": "gem",
                "ingredients": {"传说黄宝石": 2, "神话宝石碎片": 3, "金锭": 6, "成神之证": 1},
            ...
            "创世黄宝石": {
                "type": "gem",
                "ingredients": {"神话黄宝石": 2, "创世宝石碎片": 3, "金锭": 8, "成神之证": 2},
            ...
            "格挡宝石": {
                "type": "gem",
                "ingredients": {"创世青宝石": 2, "精金锭": 10, "奥金锭": 5},
            ...
            "魔法戒指": {
                "type": "accessory",
                "ingredients": {"金锭": 3, "魔法水晶": 2, "元素精华": 2},
                "result": {"item": "魔法戒指", "quantity": 1},
                "description": "增加15点魔法攻击力"
            },
            "防御戒指": {
                "type": "accessory",
                "ingredients": {"银锭": 3, "魔法水晶碎片": 5, "守护精华": 2},
                "result": {"item": "防御戒指", "quantity": 1},
                "description": "增加10点防御力"
            },
            "生命戒指": {
                "type": "accessory",
                "ingredients": {"金锭": 2, "生命之花": 3, "自然精华": 2},
                "result": {"item": "生命戒指", "quantity": 1},
                "description": "增加50点生命值"
            },
            "速度戒指": {
                "type": "accessory",
                "ingredients": {"秘银锭": 2, "风之精华": 3, "轻盈羽毛": 5},
                "result": {"item": "速度戒指", "quantity": 1},
                "description": "增加10点速度"
            },
            "幸运护符": {
                "type": "accessory",
                "ingredients": {"金锭": 2, "幸运草": 5, "精灵之尘": 3},
    ...
    def explore_area(self):
        ...
        if event_type == 'enemy' and scene['enemies']:
            enemy_name = random.choice(scene['enemies'])
            self.gui.root.after(1750, lambda: self.start_battle(enemy_name))
            ...
            def delayed_item():
                ...
                self.gui.update_game_info()
            self.gui.root.after(1750, delayed_item)
        elif event_type == 'event' and scene['events']:
            event = random.choice(scene['events'])
            self.gui.root.after(1750, lambda: self.trigger_event(event))
        else:
            self.gui.root.after(1750, lambda: self.add_message("你没有发现任何特别的东西。", 'info'))
        
        if random.random() < 0.25:  # 提高至25%
            ore_roll = random.random()
            if ore_roll < 0.35:
                self.player.add_item("铜矿石", random.randint(1, 3), self)
            elif ore_roll < 0.55:
                self.player.add_item("铁矿石", random.randint(1, 2), self)
            elif ore_roll < 0.70:
                self.player.add_item("银矿石", 1, self)
            elif ore_roll < 0.82:
                self.player.add_item("金矿石", 1, self)
            elif ore_roll < 0.90:
                self.player.add_item("秘银矿石", 1, self)
            elif ore_roll < 0.95:
                self.player.add_item("精金矿石", 1, self)
            elif ore_roll < 0.98:
                self.player.add_item("奥金矿石", 1, self)
            else:
                self.player.add_item("星铁矿石", 1, self)  # 2%概率
                self.add_message("🌟 天哪！你发现了极其稀有的星铁矿石！", 'gold')
        
        # 宝石碎片获取（含高阶碎片）
        if random.random() < 0.15:  # 15%概率获得宝石碎片
            gem_roll = random.random()
            if gem_roll < 0.50:  # 50% → 普通
                self.player.add_item("普通宝石碎片", random.randint(1, 3), self)
                self.add_message("你发现了一些宝石碎片！", 'success')
            elif gem_roll < 0.75:  # 25% → 魔法
                self.player.add_item("魔法宝石碎片", random.randint(1, 2), self)
                self.add_message("你发现了一些魔法宝石碎片！", 'success')
            elif gem_roll < 0.90:  # 15% → 稀有
                self.player.add_item("稀有宝石碎片", 1, self)
                self.add_message("你发现了一些稀有宝石碎片！", 'success')
            elif gem_roll < 0.96:  # 6% → 史诗
                self.player.add_item("史诗宝石碎片", 1, self)
                self.add_message("🌟 你发现了稀有的史诗宝石碎片！", 'gold')
            elif gem_roll < 0.99:  # 3% → 传说
                self.player.add_item("传说宝石碎片", 1, self)
                self.add_message("✨ 你发现了极其稀有的传说宝石碎片！", 'gold')
            else:  # 1% → 神话
                self.player.add_item("神话宝石碎片", 1, self)
                self.add_message("👑 天哪！你发现了传说中的神话宝石碎片！！！", 'gold')
        
        exp_gained = random.randint(1, 5)
        def delayed_exp():
            self.player.gain_exp(exp_gained)
            self.add_message(f"探索获得 {exp_gained} 点经验值", 'info')
        self.gui.root.after(1750, delayed_exp)
    ...
    def start_battle(self, enemy_name, active_battle=False):
        ...
        def _insert_message(msg, tag=None):
            message_text.insert(tk.END, msg + "\n", tag)
            message_text.see(tk.END)
            dialog.update()
        
        def add_battle_message(msg, tag=None, delay=0):
            """添加战斗消息（支持延迟）"""
            if delay > 0:
                dialog.after(delay, lambda: _insert_message(msg, tag))
            else:
                _insert_message(msg, tag)
        ...
        def teammate_attack(teammate):
            ...
            if random.random() < 0.1:
                ...
                add_battle_message(f"⚡ {teammate['name']} 暴击！", 'warning', 750)
            ...
            add_battle_message(f"{teammate['name']} 使用攻击，对 {enemy_name} 造成了 {damage} 点伤害！", 'info', 750)
        ...
        def physical_attack():
            ...
            if is_crit:
                ...
                add_battle_message(f"⚡ 暴击！", 'warning', 750)
            ...
            if lifesteal > 0 and damage > 0:
                ...
                add_battle_message(f"💚 吸血恢复 {heal} 点生命", 'success', 750)
            ...
            add_battle_message(f"你使用物理攻击，对 {enemy_name} 造成了 {damage} 点伤害！", 'success', 750)
            enemy_hp_label.config(text=f"👾 {enemy_name} HP: {current_enemy_hp}/{enemy_hp}")
            ...
            for pet in self.pets:
                ...
                if pet['loyalty'] >= 30:
                    ...
                    if random.random() < 0.2 and pet['skills']:
                        ...
                        add_battle_message(f"🐾 {pet['name']} 使用 {skill}！", 'info', 750)
                    ...
                    add_battle_message(f"🐾 {pet['name']} 攻击了 {enemy_name}，造成 {damage} 点伤害！", 'info', 750)
                    enemy_hp_label.config(text=f"👾 {enemy_name} HP: {current_enemy_hp}/{enemy_hp}")
        ...
        def magic_attack():
            ...
            if not self.player.magic_affinity:
                add_battle_message("你还未掌握魔法力量！", 'warning', 750)
                return
            ...
            if weapon and weapon in self.items:
                ...
                if 'element' in weapon_info:
                    ...
                    if element == 'fire' and random.random() < 0.3:
                        add_battle_message(f"🔥 燃烧效果！敌人每回合受到额外伤害", 'warning', 750)
                    elif element == 'ice' and random.random() < 0.2:
                        add_battle_message(f"❄️ 冰冻效果！敌人速度降低", 'info', 750)
                    elif element == 'lightning' and random.random() < 0.25:
                        add_battle_message(f"⚡ 麻痹效果！敌人可能无法行动", 'warning', 750)
            
            add_battle_message(f"你使用魔法攻击，对 {enemy_name} 造成了 {damage} 点伤害！", 'info', 750)
            enemy_hp_label.config(text=f"👾 {enemy_name} HP: {current_enemy_hp}/{enemy_hp}")
        ...
        def use_item():
            ...
            if not battle_items:
                add_battle_message("没有可用的物品！", 'warning', 750)
                return
            ...
            quantity_frame = tk.Frame(item_dialog, bg=self.gui.colors['bg'])
            quantity_frame.pack(pady=5)
            ...
            def on_item_select():
                ...
                if selection:
                    ...
                    item_quantity = battle_items[index][1]
                    item_info = battle_items[index][2]
                    
                    try:
                        use_qty = int(quantity_var.get())
                        use_qty = min(item_quantity, max(1, use_qty))
                    except ValueError:
                        use_qty = 1
                    ...
                    for i in range(use_qty):
                        ...
                        if effect_result:
                            for msg in effect_result['messages']:
                                add_battle_message(msg, effect_result.get('tag', 'info'), 750)
        ...
        def run_away():
            ...
            if random.random() < escape_chance:
                ...
                add_battle_message("你成功逃跑了！", 'info', 750)
                self.gui.update_game_info()
                dialog.after(1500, dialog.destroy)
            else:
                add_battle_message("逃跑失败！", 'error', 750)
                dialog.after(1750, enemy_attack_turn)
        ...
        def capture_monster():
            ...
            if len(self.pets) >= self.max_pet_size:
                add_battle_message("宠物栏已满，无法捕获更多宠物！", 'warning', 750)
                return
            
            if enemy_name not in self.capturable_monsters:
                add_battle_message("该怪物无法被捕获！", 'warning', 750)
                return
            ...
            if random.random() < adjusted_chance:
                ...
                add_battle_message(f"🎉 成功捕获 {monster_data['name']}！", 'success', 750)
                battle_running = False
                dialog.after(2000, dialog.destroy)
            else:
                add_battle_message("捕获失败！怪物逃跑了！", 'error', 750)
                dialog.after(1750, enemy_attack_turn)
        ...
        def enemy_attack_turn():
            ...
            if target_type == 'player':
                ...
                if random.random() < (dodge_chance / 100):
                    add_battle_message(f"💨 你闪避了敌人的攻击！", 'success', 750)
                    return
                ...
                if random.random() < (block_chance / 100):
                    ...
                    add_battle_message(f"🛡️ 格挡成功！伤害减半", 'info', 750)
                ...
                if thorns > 0 and damage > 0:
                    ...
                    add_battle_message(f"💥 反伤效果对敌人造成 {reflect_damage} 点伤害", 'warning', 750)
                
                self.player.hp = max(0, self.player.hp - damage)
                
                add_battle_message(f"{enemy_name} 对你造成了 {damage} 点伤害！", 'error', 1750)
                total_hp = self.player.max_hp + self.player.gem_bonus_hp + self.player.equipment_bonus_hp
                player_hp_label.config(text=f"⚔️ {self.player.name} HP: {self.player.hp}/{total_hp}")
                ...
                if alive_teammates:
                    ...
                    add_battle_message(f"{enemy_name} 对 {target_teammate['name']} 造成了 {damage} 点伤害！", 'error', 1750)
                    
                    for teammate, label in teammate_labels:
                        if teammate == target_teammate:
                            max_hp = teammate.get('hp', 100)
        ...
        def battle_victory():
            ...
            add_battle_message(f"🎉 战斗胜利！获得 {exp_gained} 经验值和 {gold_gained} 金币", 'success', 750)
            ...
            if 'drops' in enemy_data and enemy_data['drops']:
                for drop_item in enemy_data['drops']:
                    if random.random() < diff_settings['item_drop_chance']:
                        ...
                        add_battle_message(f"🎁 {enemy_name} 掉落了 {drop_item}！", 'success', 750)
            
            current_scene = self.scenes[self.current_scene]
            if current_scene.get('is_boss', False) and 'boss_name' in current_scene and enemy_name == current_scene['boss_name']:
                dimension = current_scene.get('dimension', 'mainland')
                if dimension == 'mainland':
                    self.player.add_item("主大陆凭证", 1, self)
                    add_battle_message("🎁 你获得了主大陆凭证！", 'success', 750)
                elif dimension == 'underground':
                    self.player.add_item("地下凭证", 1, self)
                    add_battle_message("🎁 你获得了地下凭证！", 'success', 750)
                elif dimension == 'sky':
                    self.player.add_item("天空凭证", 1, self)
                    add_battle_message("🎁 你获得了天空凭证！", 'success', 750)
                elif dimension == 'time':
                    self.player.add_item("时间凭证", 1, self)
                    add_battle_message("🎁 你获得了时间凭证！", 'success', 750)
                elif dimension == 'dream':
                    self.player.add_item("梦境凭证", 1, self)
                    add_battle_message("🎁 你获得了梦境凭证！", 'success', 750)

            if enemy_name == "创世神":
                self.new_game_plus_unlocked = True
                add_battle_message("🌟 你击败了创世神！二周目已解锁！", 'gold', 750)
                add_battle_message("🌍 时间维度、梦境维度和暗影维度现在可以进入！", 'gold', 750)
                self.unlock_achievement("世界守护者")
            
            if enemy_name == "暗影君主":
                add_battle_message("🎉 恭喜你击败了最终Boss！", 'success', 750)
                add_battle_message("🎆 你通关了游戏！", 'gold', 750)
                add_battle_message("🏆 你成为了真正的英雄！", 'gold', 750)
                self.unlock_achievement("游戏通关")
            
            for teammate, label in teammate_labels:
                if teammate['hp'] > 0:
                    teammate['affection'] = min(100, teammate['affection'] + 5)
                    add_battle_message(f"❤️ {teammate['name']} 的好感度增加了5点！", 'info', 750)
            ...
            for pet in self.pets:
                ...
                if pet['experience'] >= 100 * pet['level']:
                    ...
                    add_battle_message(f"🐾 {pet['name']} 升级了！现在是 {pet['level']} 级！", 'success', 750)
                pet['loyalty'] = min(100, pet['loyalty'] + 3)
                add_battle_message(f"❤️ {pet['name']} 的忠诚度增加了3点！", 'info', 750)
        ...
        def battle_defeat():
            ...
            # ===== 究极模式：永久死亡 =====
            if self.config['difficulty'] == 'ultimate':  # 改为 self.config
                add_battle_message("💀 究极模式：你被击败了！永久死亡！", 'error', 750)
                add_battle_message("🗑️ 存档将被删除...", 'error', 1500)
                
                # 删除存档
                if hasattr(self, 'current_save') and self.current_save:  # 改为 self
                    save_path = os.path.join(self.saves_dir, self.current_save)  # 改为 self
                    try:
                        if os.path.exists(save_path):
                            os.remove(save_path)
                            add_battle_message("📁 存档已删除！", 'warning', 2000)
                    except Exception as e:
                        print(f"删除存档失败: {e}")
                
                # 停止体力恢复线程
                self.stamina_recovery_running = False  # 改为 self
                if self.stamina_recovery_thread and self.stamina_recovery_thread.is_alive():  # 改为 self
                    try:
                        self.stamina_recovery_thread.join(timeout=0.5)
                    except:
                        pass
                ...
                def game_over():
                    try:
                        ...
                        pass
                    messagebox.showinfo("💀 游戏结束", 
                        f"究极模式：{self.player.name} 已永久死亡！\n"
                        f"等级: {self.player.level}\n"
                        f"击败敌人: {self.enemies_defeated}\n"
                        f"存活天数: {self.day_count}\n\n"
                        "存档已被删除。")
                    self.game_state = "menu"  # 改为 self
                    self.player = None  # 改为 self
                    if self.gui:  # 改为 self
                        self.gui.clear_display()
                        self.gui.show_main_menu()
                        self.gui.update_game_info()
                
                dialog.after(3000, game_over)
                return
            
            # ===== 普通模式：损失金币复活 =====
            self.player.hp = 1  # 改为 self
            gold_lost = min(50, self.player.gold)  # 改为 self
            self.player.gold = max(0, self.player.gold - 50)  # 改为 self
            add_battle_message(f"💀 你被击败了！损失了 {gold_lost} 金币，勉强活了下来。", 'error', 750)
            
            self.leaderboard['combat_stats']['battles_lost'] += 1  # 改为 self
            
            self.gui.update_game_info()  # 改为 self
            dialog.after(2000, dialog.destroy)
        
        # 按钮防连点：点击后立即禁用按钮，2000ms后恢复
        def make_battle_handler(cmd):
            def handler():
                # 立即禁用所有按钮
                ...
                # 2000ms后恢复所有按钮
                dialog.after(2000, lambda: restore_buttons())
                # 立即执行命令（命令内部的消息会自己延迟）
                cmd()
            return handler
        ...
        physical_btn = tk.Button(
            action_frame,
            text="⚔️ 物理攻击",
            command=make_battle_handler(physical_attack),
            font=self.gui.normal_font,
            bg=self.gui.colors['button_bg'],
            fg=self.gui.colors['button_fg'],
            width=10
        ...
        magic_btn = tk.Button(
            action_frame,
            text="🔮 魔法攻击",
            command=make_battle_handler(magic_attack),
            font=self.gui.normal_font,
            bg=self.gui.colors['button_bg'],
            fg=self.gui.colors['button_fg'],
            width=10
        ...
        item_btn = tk.Button(
            action_frame,
            text="🧪 使用物品",
            command=make_battle_handler(use_item),
            font=self.gui.normal_font,
            bg=self.gui.colors['button_bg'],
            fg=self.gui.colors['button_fg'],
            width=10
        ...
        capture_btn = tk.Button(
            action_frame,
            text="🐾 捕获",
            command=make_battle_handler(capture_monster),
            font=self.gui.normal_font,
            bg=self.gui.colors['button_bg'],
            fg=self.gui.colors['button_fg'],
            width=10
        ...
        run_btn = tk.Button(
            action_frame,
            text="🏃 逃跑",
            command=make_battle_handler(run_away),
            font=self.gui.normal_font,
            bg=self.gui.colors['button_bg'],
            fg=self.gui.colors['button_fg'],
            width=10
    ...
    def perform_unique_action(self, action):
        ...
        # ===== 辅助函数：检测死亡 =====
        def apply_damage(damage, action_name):
            """造成伤害并检测死亡"""
            if self.player.hp <= damage:
                self.player.hp = 0
                self.add_message(f"你受到了 {damage} 点伤害！", 'error')
                self.handle_player_death(f"在{action_name}中死亡")
                return True
            else:
        ...
        if action == "采集熔岩样本":
            if random.random() < 0.7:
                ...
                if apply_damage(random.randint(10, 30), action):
                    return
            ...
            if self.player.level >= 25:
                ...
                if apply_damage(random.randint(20, 40), action):
                    return
        ...
        # 时间维度 - 时间虚空操作
            ...
            if self.player.level >= 35:
                self.add_message("你向时间领主发起了挑战，时间的流转在此刻凝固！", 'warning')
                self.start_battle("时间领主")
            else:
        ...
        # 梦境维度 - 梦境核心操作
            ...
            if self.player.level >= 40:
                self.add_message("你向梦境主宰发起了挑战，梦境与现实的界限开始崩塌！", 'warning')
                self.start_battle("梦境主宰")
            else:
        ...
        # 古代图书馆 - 与书籍交流
            ...
            if random.random() < 0.55:
                ...
                self.add_message(f"与书籍的深度交流让你获得了很多知识，获得 {exp_gain} 点经验值！", 'info')
            else:
                self.add_message("书籍的内容太过深奥，你一时难以理解。", 'info')

        # 龙穴 - 龙的试炼
        elif action == "龙的试炼":
            if self.player.level >= 30:
                if random.random() < 0.5:
                    self.player.add_item("龙之心", 1, self)
                    self.player.gain_exp(80)
                    self.add_message("你通过了龙的试炼，获得龙之心和80点经验值！", 'success')
                else:
                    if apply_damage(40, action):
                        return
            else:
        ...
        # 未来都市 - 时空旅行
            ...
            if random.random() < 0.4:
                self.player.add_item("时空碎片", 1, self)
                self.player.gain_exp(60)
                self.add_message("时空旅行让你窥见了时间的奥秘，获得时空碎片和60点经验值！", 'success')
            else:
                if apply_damage(25, action):
                    return
        ...
        # 时间虚空 - 时空穿越
            ...
            if random.random() < 0.35:
                ...
                self.add_message(f"时空穿越成功！你从时间裂缝中带回了时空碎片和 {gold_gain} 金币！", 'success')
            else:
                if apply_damage(40, action):
                    return
        ...
        # 暗影试炼 - 挑战暗影君主（最终Boss战）
            ...
            if self.player.level >= 50:
                self.add_message("你向暗影君主发起了最终挑战！这是决定世界命运的战斗！", 'warning')
                self.start_battle("暗影君主")
            else:
                self.add_message("你的等级还不足以挑战暗影君主，需要达到50级。", 'warning')
    ...
    def show_crafting_system(self):
        """显示合成系统 - 增强版，包含所有材料的合成"""
        if not self.player:
            messagebox.showinfo("提示", "请先开始游戏！")
            return
        
        dialog = tk.Toplevel(self.gui.root)
        dialog.title("合成系统 - 所有材料合成")
        # ========== 全屏/窗口切换 ==========
        # 默认全屏
        is_fullscreen = True
        if is_fullscreen:
        ...
        # ========== 1. 智能材料合成 - 自动识别所有材料 ==========
        self._create_material_synthesis_tab(notebook)
        
        # ========== 2. 预设合成配方 ==========
        self._create_preset_crafting_tab(notebook)
        
        # ========== 3. 预设锻造配方 ==========
        self._create_preset_smithing_tab(notebook)
        
        # ========== 4. 预设宝石配方 ==========
        self._create_preset_gem_tab(notebook)
        self._create_gem_socket_tab(notebook)
    ...
    def _create_material_synthesis_tab(self, notebook):
        """创建智能材料合成标签页 - 自动识别所有材料物品"""
        # 收集所有类型为material的物品
        material_items = {}
        for item_name, item_info in self.items.items():
            if item_info.get('type') == 'material':
                material_items[item_name] = item_info
        ...
        for name, info in material_items.items():
            value = info.get('value', 0)
            if '碎片' in name:
                material_groups["宝石碎片"].append((name, info))
            elif '精华' in name:
                material_groups["精华材料"].append((name, info))
            elif '矿石' in name or '矿' in name:
                if value >= 300:
                    material_groups["高级材料"].append((name, info))
                else:
                    material_groups["矿石材料"].append((name, info))
            elif value >= 150:
                material_groups["特殊材料"].append((name, info))
            else:
                material_groups["基础材料"].append((name, info))
        
        material_frame = tk.Frame(notebook, bg='#1e1e1e')
        notebook.add(material_frame, text="🧪 智能合成")
        
        # 顶部信息
        info_label = tk.Label(
            material_frame,
            text="💡 所有材料都可以合成！选择下方的材料进行合成",
            font=self.gui.normal_font,
            fg=self.gui.colors['info'],
            bg='#1e1e1e'
        )
        ...
        # 创建一个可滚动的Canvas
        canvas = tk.Canvas(material_frame, bg='#1e1e1e', highlightthickness=0)
        scrollbar = tk.Scrollbar(material_frame, orient='vertical', command=canvas.yview)
        scrollable_frame = tk.Frame(canvas, bg='#1e1e1e')
        ...
        for group_name, items in material_groups.items():
            ...
            # 每行显示8 个材料
            row_frame = tk.Frame(group_frame, bg='#1e1e1e')
            row_frame.pack(fill='x', padx=5, pady=5)
            
            col = 0
            max_cols = 8
            
            for material_name, material_info in items:
                if material_name == "木板":
                    continue
                # 检查玩家是否拥有该材料
                owned = self.player.inventory.get(material_name, 0)
                
                material_btn_frame = tk.Frame(row_frame, bg='#1e1e1e')
                material_btn_frame.grid(row=0, column=col, padx=3, pady=3, sticky='nsew')
                
                btn_text = f"{material_name}\n[{owned}个]"
                btn_color = self.gui.colors['button_bg'] if owned > 0 else '#3a3a3a'
                
                material_btn = tk.Button(
                    material_btn_frame,
                    text=btn_text,
                    command=lambda m=material_name, info=material_info: self._synthesize_material(m, info),
                    font=self.gui.small_font,
                    bg=btn_color,
                    fg=self.gui.colors['button_fg'],
                    width=15,
                    height=2,
                    state='normal' if owned > 0 else 'disabled'
                )
                material_btn.pack()
                ...
                value_label = tk.Label(
                    material_btn_frame,
                    text=f"💰 {material_info.get('value', 0)}",
                    font=self.gui.small_font,
                    fg=self.gui.colors['fg'],
                    bg='#1e1e1e'
                )
        ...
        # 显示玩家当前拥有的材料数量
        total_materials = sum(1 for item in self.player.inventory.keys() 
                             if item in self.items and self.items[item].get('type') == 'material')
        material_count_label = tk.Label(
            material_frame,
            text=f"📦 已拥有材料种类: {total_materials}",
            font=self.gui.small_font,
            fg=self.gui.colors['info'],
            bg='#1e1e1e'
        )
        material_count_label.pack(pady=5)

    def _synthesize_material(self, material_name, material_info):
        """智能合成单个材料 - 每次合成消耗100金币"""
        # 检查金币是否足够
        if self.player.gold < 100:
            messagebox.showerror("错误", f"金币不足！智能合成需要花费100金币。\n当前金币: {self.player.gold}")
            return
        
        # 根据材料名称和类型决定合成结果
        value = material_info.get('value', 0)
        result_items = []
        
        # ===== 矿石类合成 =====
        if '矿石' in material_name or '矿' in material_name:
            if '铜矿石' in material_name:
                result_items = [("铜锭", 1)]
            elif '铁矿石' in material_name:
                result_items = [("铁锭", 1)]
            elif '银矿石' in material_name:
                result_items = [("银锭", 1)]
            elif '金矿石' in material_name:
                result_items = [("金锭", 1)]
            elif '秘银矿石' in material_name:
                result_items = [("秘银锭", 1)]
            elif '精金矿石' in material_name:
                result_items = [("精金锭", 1)]
            elif '奥金矿石' in material_name:
                result_items = [("奥金锭", 1)]
            elif '星铁矿石' in material_name:
                result_items = [("星铁锭", 1)]
            else:
                # 通用矿石合成：2个矿石 -> 1个锭 + 少量金币
                result_items = [("矿石锭", 1), ("金币", value // 10)]
        
        # ===== 精华类合成 =====
        elif '精华' in material_name:
            # 3个同类型精华 -> 更高级的精华或材料
            if '火焰' in material_name:
                result_items = [("火焰结晶", 1)]
            elif '冰霜' in material_name:
                result_items = [("冰霜结晶", 1)]
            elif '雷电' in material_name:
                result_items = [("雷电结晶", 1)]
            elif '自然' in material_name:
                result_items = [("自然结晶", 1)]
            elif '暗影' in material_name:
                result_items = [("暗影结晶", 1)]
            elif '光明' in material_name:
                result_items = [("光明结晶", 1)]
            else:
                result_items = [("魔法水晶", 1)]
        
        # ===== 宝石碎片合成 =====
        elif '碎片' in material_name:
            if '普通宝石碎片' in material_name:
                # 5个碎片 -> 随机普通宝石
                gem_types = ["普通红宝石", "普通蓝宝石", "普通绿宝石", "普通黄宝石", "普通紫宝石"]
                result_items = [(random.choice(gem_types), 1)]
            elif '魔法宝石碎片' in material_name:
                gem_types = ["魔法红宝石", "魔法蓝宝石", "魔法绿宝石", "魔法黄宝石", "魔法紫宝石"]
                result_items = [(random.choice(gem_types), 1)]
            elif '稀有宝石碎片' in material_name:
                gem_types = ["稀有红宝石", "稀有蓝宝石", "稀有绿宝石", "稀有黄宝石", "稀有紫宝石"]
                result_items = [(random.choice(gem_types), 1)]
            elif '史诗宝石碎片' in material_name:
                gem_types = ["史诗红宝石", "史诗蓝宝石", "史诗绿宝石", "史诗黄宝石", "史诗紫宝石"]
                result_items = [(random.choice(gem_types), 1)]
            elif '传说宝石碎片' in material_name:
                gem_types = ["传说红宝石", "传说蓝宝石", "传说绿宝石", "传说黄宝石", "传说紫宝石"]
                result_items = [(random.choice(gem_types), 1)]
            elif '神话宝石碎片' in material_name:
                gem_types = ["神话红宝石", "神话蓝宝石", "神话绿宝石", "神话黄宝石", "神话紫宝石"]
                result_items = [(random.choice(gem_types), 1)]
            elif '创世宝石碎片' in material_name:
                gem_types = ["创世红宝石", "创世蓝宝石", "创世绿宝石", "创世黄宝石", "创世紫宝石"]
                result_items = [(random.choice(gem_types), 1)]
            else:
                result_items = [("宝石", 1)]
        
        # ===== 龙类材料合成 =====
        elif '龙鳞' in material_name:
            if '远古' in material_name:
                result_items = [("远古龙鳞甲", 1)]
            else:
                result_items = [("龙鳞甲", 1)]
        elif '龙血' in material_name:
            result_items = [("龙血药水", 2)]
        elif '龙心' in material_name:
            result_items = [("龙心药剂", 1)]
        
        # ===== 皮革/布料类合成 =====
        elif '皮革' in material_name or '毛皮' in material_name:
            result_items = [("皮甲", 1)]
        elif '蛛丝' in material_name or '丝' in material_name:
            result_items = [("丝线", 2)]
        
        # ===== 植物/草药类合成 =====
        elif '草药' in material_name or '花' in material_name:
            if '生命' in material_name:
                result_items = [("治疗药水", 2)]
            else:
                result_items = [("草药", 2)]
        
        # ===== 木材类合成 =====
        elif '木材' in material_name or '木' in material_name:
            result_items = [("木炭", 3)]
        
        # ===== 空瓶合成 =====
        elif '空瓶' in material_name:
            result_items = [("玻璃瓶", 1)]
        
        # ===== 默认合成 =====
        else:
            # 如果材料有craft_recipes，使用预设配方
            if 'craft_recipes' in material_info and material_info['craft_recipes']:
                recipe = random.choice(material_info['craft_recipes'])
                result_items = [(recipe, 1)]
            else:
                # 默认：3个材料 -> 1个随机物品
                possible_results = ["治疗药水", "魔法药水", "经验药水", "金币"]
                result_items = [(random.choice(possible_results), 1)]
        
        # 执行合成 - 消耗金币和1个材料
        if self.player.inventory.get(material_name, 0) > 0:
            # 消耗100金币
            self.player.gold -= 100
            
            # 消耗1个材料
            self.player.remove_item(material_name, 1)
            
            # 添加合成结果
            for result_name, quantity in result_items:
                if result_name == "金币":
                    self.player.gold += quantity
                    self.add_message(f"💰 合成获得 {quantity} 金币！", 'success')
                else:
                    self.player.add_item(result_name, quantity)
                    self.add_message(f"⚒️ 合成成功！消耗100金币，将 {material_name} 合成为 {result_name} x{quantity}", 'success')
            ...
            messagebox.showerror("错误", "你没有这个材料！")

    def _create_preset_crafting_tab(self, notebook):
        """创建预设合成配方标签页 (原有的合成系统)"""
        from tkinter import ttk
        
        frame = tk.Frame(notebook, bg='#1e1e1e')
        notebook.add(frame, text="📋 预设合成")
        ...
        for type_name, recipes in recipe_types.items():
            ...
            for recipe_name, recipe_data in recipes:
                ...
                recipe_frame = tk.Frame(type_frame, bg='#1e1e1e')
                ...
                desc_label = tk.Label(
                    recipe_frame,
                    text=recipe_data['description'][:30] + ("..." if len(recipe_data['description']) > 30 else ""),
                    font=self.gui.small_font,
                    fg=self.gui.colors['fg'],
                    bg='#1e1e1e',
                    width=20,
                ...
                for material, amount in recipe_data['ingredients'].items():
                    ...
                    status = "✅" if current >= amount else f"❌({current}/{amount})"
                    material_status.append(f"{material}:{status}")
                
                status_label = tk.Label(
                    recipe_frame,
                    text=" ".join(material_status),
                    font=self.gui.small_font,
                    fg=self.gui.colors['info'],
                    bg='#1e1e1e'
                )
                ...
                # 合成按钮
                def craft(r_name=recipe_name, r_data=recipe_data):
                    # 检查材料
                    for material, amount in r_data['ingredients'].items():
                        if self.player.inventory.get(material, 0) < amount:
                            messagebox.showerror("错误", f"材料不足！需要 {material} x{amount}")
                            return
                    ...
                    self.player.crafting_count = getattr(self.player, 'crafting_count', 0) + 1
                    self.gui.update_game_info()
                    # 刷新界面
                ...
                craft_btn.pack(side='right', padx=5)
        
        canvas.pack(side='left', fill='both', expand=True)
        scrollbar.pack(side='right', fill='y')

    def _create_preset_smithing_tab(self, notebook):
        """创建预设锻造配方标签页 - 每次锻造消耗1块木炭"""
        from tkinter import ttk
        
        frame = tk.Frame(notebook, bg='#1e1e1e')
        notebook.add(frame, text="🔨 预设锻造")
        
        # 显示木炭库存
        charcoal_count = self.player.inventory.get("木炭", 0)
        charcoal_info = tk.Label(
            frame,
            text=f"🔥 木炭库存: {charcoal_count} 块 (每次锻造消耗1块)",
            font=self.gui.normal_font,
            fg=self.gui.colors['info'],
            bg='#1e1e1e'
        )
        charcoal_info.pack(pady=5)
        ...
        for type_name, recipes in recipe_types.items():
            ...
            for recipe_name, recipe_data in recipes:
                ...
                # 检查木炭
                charcoal_current = self.player.inventory.get("木炭", 0)
                if charcoal_current < 1:
                    has_materials = False
                    missing_materials.append("木炭 x1")
                
                # 检查配方材料
                for material, amount in recipe_data['ingredients'].items():
                    if material not in self.player.inventory or self.player.inventory[material] < amount:
                        has_materials = False
                        missing_materials.append(f"{material} x{amount}")
                
                recipe_frame = tk.Frame(type_frame, bg='#1e1e1e')
                recipe_frame.pack(fill='x', padx=5, pady=1)
                ...
                desc_label = tk.Label(
                    recipe_frame,
                    text=recipe_data['description'][:25] + ("..." if len(recipe_data['description']) > 25 else ""),
                    font=self.gui.small_font,
                    fg=self.gui.colors['fg'],
                    bg='#1e1e1e',
                    width=18,
                ...
                desc_label.pack(anchor='w', padx=10, pady=(0, 1), fill='x')
                ...
                # 添加木炭
                charcoal_current = self.player.inventory.get("木炭", 0)
                color = self.gui.colors['success'] if charcoal_current >= 1 else self.gui.colors['warning']
                material_parts.append(f"🔥木炭 1/{charcoal_current}")
                ...
                def forge(r_name=recipe_name, r_data=recipe_data):
                    # 检查木炭
                    if self.player.inventory.get("木炭", 0) < 1:
                        messagebox.showerror("错误", "木炭不足！每次锻造需要消耗1块木炭。\n请先获取木炭（可通过合成木炭获得）。")
                        return
                    ...
                    self.add_message(f"🔨 锻造成功！消耗1块木炭，获得 {result_item} x{result_quantity}", 'success')
                    
                    self.player.smithing_count = getattr(self.player, 'smithing_count', 0) + 1
                    self.gui.update_game_info()
                ...
                # 判断按钮状态
                btn_state = 'normal' if has_materials else 'disabled'
                forge_btn = tk.Button(
                    recipe_frame,
                    text="锻造",
                    command=forge,
                ...
                forge_btn.pack(side='right', padx=5)
                
                # 如果材料不足，显示提示
                if not has_materials:
                    tip_label = tk.Label(
                        recipe_frame,
                        text=f"❌ 缺: {', '.join(missing_materials)}",
                        font=self.gui.small_font,
                        fg=self.gui.colors['warning'],
                        bg='#1e1e1e'
                    )
    ...
    def _create_preset_gem_tab(self, notebook):
        """创建预设宝石合成标签页"""
        from tkinter import ttk
        
        frame = tk.Frame(notebook, bg='#1e1e1e')
        notebook.add(frame, text="💎 预设宝石")
        ...
        for quality_name, recipes in gem_types.items():
            ...
            for recipe_name, recipe_data in recipes:
                ...
                desc_label = tk.Label(
                    recipe_frame,
                    text=recipe_data['description'][:20] + ("..." if len(recipe_data['description']) > 20 else ""),
                    font=self.gui.small_font,
                    fg=self.gui.colors['fg'],
                    bg='#1e1e1e',
                    width=15,
                ...
                def make_gem(r_name=recipe_name, r_data=recipe_data):
                    for material, amount in r_data['ingredients'].items():
                        if self.player.inventory.get(material, 0) < amount:
                            messagebox.showerror("错误", f"材料不足！需要 {material} x{amount}")
                            return
                    ...
                    self.player.gem_count = getattr(self.player, 'gem_count', 0) + 1
                    self.gui.update_game_info()
                    notebook.master.destroy()
                    self.show_crafting_system()
                ...
                gem_btn.pack(side='right', padx=5)
        
        canvas.pack(side='left', fill='both', expand=True)
        scrollbar.pack(side='right', fill='y')
    def _get_gem_effect_description(self, item_info):
        """获取宝石效果的描述文字"""
        effect = item_info.get('effect', '')
        value = item_info.get('value', 0)
        ...
        return effect_map.get(effect, item_info.get('description', '未知效果'))
    def _create_gem_socket_tab(self, notebook):
        """创建宝石镶嵌标签页"""
        from tkinter import ttk
        
        frame = tk.Frame(notebook, bg='#1e1e1e')
        notebook.add(frame, text="💍 宝石镶嵌")
        ...
        equipment_frame.pack(fill='x', padx=3, pady=3)
        
        self.socket_equip_vars = {}
        slot_labels = {
            "weapon": "⚔️ 武器",
            "armor": "🛡 盔甲",
            "accessory": "💍 饰品"
        ...
        for slot_name in ["weapon", "armor", "accessory"]:
            ...
            slot_frame.pack(fill='x', padx=3, pady=1)
            
            tk.Label(
                slot_frame,
                text=f"{slot_labels.get(slot_name, slot_name)}:",
                font=self.gui.small_font,
                fg=self.gui.colors['fg'],
                bg='#1e1e1e',
                width=8,
            ...
            equip_name = self.player.equipped.get(slot_name, '无')
            tk.Label(
                slot_frame,
                text=equip_name,
                font=self.gui.small_font,
                fg=self.gui.colors['success'] if equip_name != '无' else self.gui.colors['warning'],
                bg='#1e1e1e',
                width=12,
                anchor='w'
            ).pack(side='left', padx=3)
            ...
            gem_text = f"💎 {current_gem or '无'}"
            gem_var = tk.StringVar(value=gem_text)
            self.socket_equip_vars[slot_name] = gem_var
            
            tk.Label(
                slot_frame,
                textvariable=gem_var,
                font=self.gui.small_font,
                fg=self.gui.colors['info'] if current_gem else self.gui.colors['warning'],
                bg='#1e1e1e',
                width=14,
                anchor='w'
            ).pack(side='left', padx=3)
        ...
        gems_frame.pack(fill='both', expand=True, padx=3, pady=3)
        
        canvas = tk.Canvas(gems_frame, bg='#1e1e1e', highlightthickness=0, height=120)
        scrollbar = tk.Scrollbar(gems_frame, orient='vertical', command=canvas.yview)
        scrollable_frame = tk.Frame(canvas, bg='#1e1e1e')
        ...
        if not gem_items:
            ...
            for item_name, quantity, item_info in gem_items:
                gem_frame = tk.Frame(scrollable_frame, bg='#1e1e1e')
                gem_frame.pack(fill='x', padx=3, pady=1)
                ...
                effect_desc = self._get_gem_effect_description(item_info)
                tk.Label(
                    gem_frame,
                    text=effect_desc,
                    font=self.gui.small_font,
        ...
        action_frame.pack(fill='x', padx=3, pady=3)
        
        # 第一行：选择槽位和宝石
        row1 = tk.Frame(action_frame, bg='#1e1e1e')
        row1.pack(pady=2)
        ...
        self.socket_slot_var = tk.StringVar(value="weapon")
        slot_combo = ttk.Combobox(
            row1,
            textvariable=self.socket_slot_var,
            values=["weapon", "armor", "accessory"],
            state="readonly",
            width=10
        )
        slot_combo.pack(side='left', padx=3)
        ...
        gem_names = [item[0] for item in gem_items] if gem_items else ["无可用宝石"]
        self.socket_gem_var = tk.StringVar(value=gem_names[0] if gem_names else "")
        gem_combo = ttk.Combobox(
            row1,
            textvariable=self.socket_gem_var,
            values=gem_names,
            state="readonly" if gem_names else "disabled",
            width=12
        )
        gem_combo.pack(side='left', padx=3)
        
        # 第二行：按钮
        row2 = tk.Frame(action_frame, bg='#1e1e1e')
        row2.pack(pady=3)
        
        def socket_gem():
            slot = self.socket_slot_var.get()
            gem_name = self.socket_gem_var.get()
            ...
            gem_effect = gem_info.get('effect', '')
            
            rules = self.gem_socket_rules.get(slot, {})
            allowed_types = rules.get('allowed_types', [])
            if allowed_types and gem_effect not in allowed_types:
                messagebox.showerror("错误", f"该宝石不能镶嵌在{slot}上！\n允许的类型: {', '.join(allowed_types)}")
                return
    ...
    def handle_scene_selection(self, scene_key, scene_data):
        """处理场景选择"""
        # 检查等级要求
        required_level = scene_data.get('required_level', 1)
        if self.player.level < required_level:
            self.add_message(f"等级不足！需要达到 {required_level} 级才能前往 {scene_data['name']}。\n当前等级：{self.player.level}", 'warning')
            return
        
        if scene_key not in self.unlocked_scenes:
            if scene_key == "shadow_trial":
                # 暗影试炼的特殊解锁条件
                if self.player.gold < 70000:
                    self.add_message(f"金币不足！需要 70000 金币。", 'warning')
                    return
                if self.player.inventory.get("暗影密钥", 0) < 1:
                    self.add_message(f"暗影密钥不足！需要 1 把暗影密钥。", 'warning')
                    return
                
                if messagebox.askyesno("解锁场景", f"确定花费 70000 金币和 1 把暗影密钥解锁 {scene_data['name']} 吗？"):
                    self.player.gold -= 70000
                    self.player.remove_item("暗影密钥", 1)
                    ...
                    self.add_message(f"建议等级：50+ | 建议装备：传说级", 'warning')
                else:
                    return
            else:
                # 普通场景的解锁逻辑
    ...
    def save_game(self, slot=None, save_name=None):
        ...
        try:
            ...
            save_data = {
                ...
                "new_game_plus_unlocked": self.new_game_plus_unlocked,  # ===== 新增 =====
                "config": {
                ...
                "game_state": {
                    ...
                    "visited_scenes": list(getattr(self, 'visited_scenes', set()))
                },
                "compendium": {
                    "enemies": self.compendium["enemies"],
                    "items": self.compendium["items"],
                    "achievements": self.compendium["achievements"],
                    "completion": self.compendium["completion"],
                    "rewards_claimed": self.compendium["rewards_claimed"]
                },
    ...
    def load_save_game(self, save_file):
        ...
        try:
            ...
            self.enemies_defeated = save_data["game_state"].get("enemies_defeated", 0)
            self.trades_completed = save_data["game_state"].get("trades_completed", 0)
            self.visited_scenes = set(save_data["game_state"].get("visited_scenes", []))
            self.new_game_plus_unlocked = save_data.get("new_game_plus_unlocked", False)
            if "config" in save_data:
                self.config["difficulty"] = save_data["config"].get("difficulty", "normal")
            # 加载图鉴数据
            if "compendium" in save_data:
                self.compendium["enemies"] = save_data["compendium"]["enemies"]
                self.compendium["items"] = save_data["compendium"]["items"]
                self.compendium["achievements"] = save_data["compendium"]["achievements"]
                self.compendium["completion"] = save_data["compendium"]["completion"]
                self.compendium["rewards_claimed"] = save_data["compendium"]["rewards_claimed"]
            ...
            if "teammates" in save_data:
                self.teammates = save_data["teammates"]
